...
**The whole e2e suite runs at both viewports**: `npm run test:e2e` (desktop) and `npm run test:e2e:mobile` (iPhone 13, 390x844) each run every spec, and CI runs both. The mobile/desktop UI split triggers at `MOBILE_BREAKPOINT` (768px, `src/hooks/use-mobile.tsx`), and it is a real split, not just reflow — the assignment matrix hides its filters behind a "Filtros" toggle, the expenses page swaps its `<table>` for a card list, and tour management moves its header actions into a "Más" action sheet. **Both layouts are in the DOM at every width** (one is `md:hidden`, the other `hidden md:block`), so a locator that does not account for the viewport will silently match the hidden copy. Use `isMobileViewport(page)` from `tests/e2e/support/app.ts` to drive whichever surface is on screen rather than assuming the desktop one.

A corollary worth remembering: an icon-only control whose label is `hidden sm:inline` has **no accessible name** below that breakpoint, so `getByRole("button", { name })` cannot find it. That is an app bug (WCAG 4.1.2), not a test bug — give the button an `aria-label` that contains the visible text.
...
- **Transport types are listed in many places — change them together.** The `transport_type` enum (trailer, 9m, 8m, 6m, 4m, furgoneta, rv, sleeper_bus) is mirrored by `REQUEST_TRANSPORT_OPTIONS` (`src/constants/transportOptions.ts`, pinned by its test), three DB checks (`transport_request_items`, `hoja_de_ruta_transport`, `truck_planner_transport_mappings`), three RPC lists (`save_transport_request`, `replace_transport_request_with_items` — which silently drops unknown types — and `tp_upsert_department_transport_request`), `create-transport-request`'s `VALID_TRANSPORT_TYPES`, the Hoja de Ruta `Transport` type and `ModernTransportSection`, and the label maps (`getLogisticsTransportTypeLabel`, push `transportRequestEmailFormat`, `hojaDeRutaExport`, `logisticsCalendarExport`, wallboard). Adding a new enum value needs its own migration before any migration that uses it. `rv` is deliberately fleet-only.
- **Transport companies are listed in several places too.** `transport_provider_enum` (DB, e.g. `the_wild_tour`, `montoya`) is mirrored by `TRANSPORT_PROVIDERS` (`src/constants/transportProviders.ts`; use `transportProviderLabel()` to display one), while Hoja de Ruta transport rows use their own `company` values (`'wild tour'`, `'sector-pro'`, …) guarded by `hoja_de_ruta_transport_company_check` and mapped in `ModernTransportSection` plus the label maps in `hojaDeRutaExport` and the HdR PDF formatters. Add a company to all of them at once.
- **Crew transfers are a logistics event type, not a travel booking.** `crew_transfer` (next to `load`/`unload`) moves people in our own or hired vehicles: `origin_location_id` is the pick-up point, `location_id` the destination, `passenger_count` who travels, and `end_date`/`end_time` (valid on any transport) keep the driver and vehicle blocked for days. Trains, planes and taxis do not belong in `transport_type`. Code that branches on `event_type` must handle all three values — use `logisticsEventTypeLabel` / `isLogisticsEventOnDay` from `@/components/logistics/logisticsEventTypes` rather than `load ? … : …`. A load/unload also carries `movement_type` (the request's *Tipo de movimiento*, inherited by trigger when planned); show both with `transportOperationLabel`. Crew transfers are never a request movement type.
- **Sleeper-bus berths belong to the bus run, not the request.** Fleet buses list their `berth_layouts`, each `sleeper_bus` event carries `berth_count`, and the job crew count comes from `get_logistics_matrix()` (`job_crew_count`) or `fetchJobCrewCount`. See `docs/workflows/logistics-driver-matrix.md`.
