...
- **Driver assignments never touch staffing either** — `transport_driver_assignments` hangs off `logistics_events`; upcoming non-declined assignments block event deletion/replanning/cancellation until explicitly removed or reassigned in the matrix, while finished/declined history may still cascade. Never create job assignments or timesheets for conductors
