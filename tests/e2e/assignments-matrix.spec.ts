...
test("carries a long-press multi-select into the offer, clamped to the job", async ({ page }) => {
  ...
  await bootstrapApp(page, {
    auth: { role: "management", department: "sound" },
    tables: {
      // The job runs today..+2; the selection below deliberately reaches +4.
      "jobs": [
        {
          id: "seed-job-1",
          title: "Fiestas de Cuenca",
  ...
  await page.getByRole("button", { name: /enviar oferta por whatsapp/i }).click();
  await page.getByRole("radio", { name: /Fiestas de Cuenca/ }).click();
  await page.getByRole("button", { name: /^Enviar oferta/ }).last().click();

  // The offer opens pre-filled on "varios días" with the two days inside the
  // job — the day outside it is dropped rather than silently offered.
  await expect(page.getByRole("radio", { name: /varios días/i })).toHaveAttribute("aria-checked", "true");
  await expect(page.getByRole("button", { name: /^Enviar oferta \(2 días\)$/ })).toBeVisible();
});
