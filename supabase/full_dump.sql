...
COMMENT ON FUNCTION "public"."check_technician_conflicts"("_technician_id" "uuid", "_target_job_id" "uuid", "_target_date" "date", "_single_day" boolean, "_include_pending" boolean) IS 'Comprehensive conflict checking that distinguishes between hard conflicts (confirmed assignments, unavailability) and soft conflicts (pending invitations). Used by both frontend and edge functions.';
...
BEGIN
  ...
  -- Resolve category for everyone (including house techs for rate selection)
  SELECT
    CASE
  ...
  -- If no category from job_assignments, try profile default (non-house only)
  IF cat IS NULL AND NOT house THEN
  ...
  -- Category is required for non-house techs
  IF cat IS NULL AND NOT house THEN
  ...
  -- Base rate lookup
  IF house THEN
    -- For house techs on tour dates, use tour-specific rates based on category
    -- If category is responsable, use tour_base_responsable_eur, otherwise use tour_base_other_eur
    -- Always fall back to base_day_eur if tour rates are not set
    IF cat = 'responsable' THEN
      SELECT COALESCE(tour_base_responsable_eur, base_day_eur) INTO base
      FROM house_tech_rates
      WHERE profile_id = _tech_id;
    ELSE
      -- For tecnico/especialista or NULL category, use tour_base_other_eur
  ...
  IF team_member THEN
    ...
    BEGIN
      ...
      -- Use correct Spanish enum: 'Cancelado' instead of 'cancelled'
...
CREATE OR REPLACE FUNCTION "public"."extras_total_for_job_tech"("_job_id" "uuid", "_tech_id" "uuid") RETURNS "jsonb"
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
    WITH extras_rows AS (
...
ALTER FUNCTION "public"."extras_total_for_job_tech"("_job_id" "uuid", "_tech_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."get_job_total_amounts"("_job_id" "uuid", "_user_role" "text" DEFAULT NULL::"text") RETURNS TABLE("job_id" "uuid", "total_approved_eur" numeric, "total_pending_eur" numeric, "breakdown_by_category" "jsonb", "individual_amounts" "jsonb", "user_can_see_all" boolean)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION "public"."get_profiles_with_skills"() RETURNS TABLE("id" "uuid", "first_name" "text", "nickname" "text", "last_name" "text", "email" "text", "role" "public"."user_role", "phone" "text", "dni" "text", "department" "text", "assignable_as_tech" boolean, "bg_color" "text", "skills" "json")
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
BEGIN
...
COMMENT ON FUNCTION "public"."remove_assignment_with_timesheets"("p_job_id" "uuid", "p_technician_id" "uuid") IS 'Deletes all per-day timesheets for a job/technician pair and removes the parent job_assignment when no coverage remains.';
...
COMMENT ON FUNCTION "public"."toggle_timesheet_day"("p_job_id" "uuid", "p_technician_id" "uuid", "p_date" "date", "p_present" boolean, "p_source" "text") IS 'Ensures a parent assignment exists and toggles a per-day timesheet row used by the staffing matrix.';
...
CREATE OR REPLACE VIEW "public"."current_stock_levels" WITH ("security_invoker"='true', "security_barrier"='true') AS
 SELECT "e"."id" AS "equipment_id",
    "e"."name" AS "equipment_name",
    "e"."category",
    "e"."department",
    COALESCE("sum"("gse"."base_quantity"), (0)::bigint) AS "current_quantity"
   FROM ("public"."equipment" "e"
     LEFT JOIN "public"."global_stock_entries" "gse" ON (("gse"."equipment_id" = "e"."id")))
  GROUP BY "e"."id", "e"."name", "e"."category", "e"."department";
...
CREATE OR REPLACE VIEW "public"."equipment_availability_with_rentals" WITH ("security_invoker"='true', "security_barrier"='true') AS
 SELECT "e"."id" AS "equipment_id",
    "e"."name" AS "equipment_name",
    "e"."category",
    "e"."department",
    COALESCE("csl"."current_quantity", (0)::bigint) AS "base_quantity",
    COALESCE(( SELECT "sum"("sr"."quantity") AS "sum"
           FROM "public"."sub_rentals" "sr"
          WHERE (("sr"."equipment_id" = "e"."id") AND ((CURRENT_DATE >= "sr"."start_date") AND (CURRENT_DATE <= "sr"."end_date")))), (0)::bigint) AS "rental_boost",
    (COALESCE("csl"."current_quantity", (0)::bigint) + COALESCE(( SELECT "sum"("sr"."quantity") AS "sum"
           FROM "public"."sub_rentals" "sr"
          WHERE (("sr"."equipment_id" = "e"."id") AND ((CURRENT_DATE >= "sr"."start_date") AND (CURRENT_DATE <= "sr"."end_date")))), (0)::bigint)) AS "total_available"
   FROM ("public"."equipment" "e"
...
CREATE TABLE IF NOT EXISTS "public"."house_tech_rates" (
...
ALTER TABLE "public"."house_tech_rates" OWNER TO "postgres";


COMMENT ON COLUMN "public"."house_tech_rates"."rehearsal_day_eur" IS 'Fixed daily rate for rehearsals. If NULL, uses normal calculation.';



COMMENT ON COLUMN "public"."house_tech_rates"."tour_base_responsable_eur" IS 'Tour base rate when house tech works as responsable on tour dates. If NULL, falls back to base_day_eur.';



COMMENT ON COLUMN "public"."house_tech_rates"."tour_base_other_eur" IS 'Tour base rate when house tech works as tecnico/especialista on tour dates. If NULL, falls back to base_day_eur.';
...
CREATE TABLE IF NOT EXISTS "public"."job_assignments" (
    ...
    CONSTRAINT "job_assignments_single_day_check" CHECK ((("single_day" = false) OR ("assignment_date" IS NOT NULL)))
);
...
COMMENT ON COLUMN "public"."job_assignments"."single_day" IS 'True when the assignment covers only a single day (assignment_date must be set). False for whole-job assignments covering the entire job span.';



COMMENT ON COLUMN "public"."job_assignments"."assignment_date" IS 'Specific date covered by this assignment when single_day=true. NULL for whole-job assignments. This is the standardized column name for single-day assignment dates.';
...
CREATE TABLE IF NOT EXISTS "public"."power_requirement_tables" (
    ...
    "department" "text",
    CONSTRAINT "valid_pdu_type" CHECK (("pdu_type" = ANY (ARRAY['CEE32A 3P+N+G'::"text", 'CEE63A 3P+N+G'::"text", 'CEE400A 3P+N+G'::"text"])))
);
...
CREATE OR REPLACE VIEW "public"."system_health_assignments" AS
...
CREATE OR REPLACE VIEW "public"."system_health_timesheets" AS
...
CREATE OR REPLACE VIEW "public"."v_job_tech_payout_2025" WITH ("security_invoker"='true', "security_barrier"='true') AS
 WITH "ts" AS (
         SELECT "t"."job_id",
            "t"."technician_id",
            "sum"(COALESCE("t"."amount_eur", (0)::numeric)) FILTER (WHERE ("t"."approved_by_manager" = true)) AS "timesheets_total_eur"
           FROM "public"."timesheets" "t"
          GROUP BY "t"."job_id", "t"."technician_id"
        ), "ex" AS (
         SELECT "q"."job_id",
            "q"."technician_id",
            (("q"."x" ->> 'total_eur'::"text"))::numeric AS "extras_total_eur",
            "q"."x" AS "extras_breakdown"
           FROM ( SELECT DISTINCT "job_rate_extras"."job_id",
                    "job_rate_extras"."technician_id",
                    "public"."extras_total_for_job_tech"("job_rate_extras"."job_id", "job_rate_extras"."technician_id") AS "x"
                   FROM "public"."job_rate_extras") "q"
        ), "job_rows" AS (
         SELECT "j"."id" AS "job_id",
            ...
            (COALESCE("ts"."timesheets_total_eur", (0)::numeric) + COALESCE("ex"."extras_total_eur", (0)::numeric)) AS "total_eur",
            "ex"."extras_breakdown",
            "public"."needs_vehicle_disclaimer"("a"."technician_id") AS "vehicle_disclaimer"
           FROM ((("public"."jobs" "j"
          ...
          WHERE ("j"."job_type" <> 'tourdate'::"public"."job_type")
        )
 SELECT "job_rows"."job_id",
    "job_rows"."technician_id",
    "job_rows"."timesheets_total_eur",
    "job_rows"."extras_total_eur",
    "job_rows"."total_eur",
    "job_rows"."extras_breakdown",
    "job_rows"."vehicle_disclaimer",
        CASE
            WHEN "job_rows"."vehicle_disclaimer" THEN 'Fuel/drive compensation may apply when using own vehicle. Coordinate with HR on a per-job basis.'::"text"
            ELSE NULL::"text"
...
CREATE OR REPLACE VIEW "public"."v_tour_job_rate_quotes_2025" WITH ("security_invoker"='true') AS
 SELECT "a"."job_id",
    "a"."technician_id",
    "j"."start_time",
    "j"."end_time",
...
ALTER TABLE ONLY "public"."house_tech_rates"
    ADD CONSTRAINT "house_tech_rates_pkey" PRIMARY KEY ("profile_id");



ALTER TABLE ONLY "public"."job_assignments"
    ADD CONSTRAINT "job_assignments_job_id_technician_id_key" UNIQUE ("job_id", "technician_id");
...
CREATE INDEX "idx_house_rates_updated" ON "public"."house_tech_rates" USING "btree" ("updated_at" DESC);
...
CREATE UNIQUE INDEX "job_assignments_single_day_unique" ON "public"."job_assignments" USING "btree" ("job_id", "technician_id", "assignment_date") WHERE (("single_day" = true) AND ("assignment_date" IS NOT NULL));



COMMENT ON INDEX "public"."job_assignments_single_day_unique" IS 'Uniqueness for single-day assignments (one row per date).';



CREATE UNIQUE INDEX "job_assignments_whole_job_unique" ON "public"."job_assignments" USING "btree" ("job_id", "technician_id") WHERE (("single_day" = false) OR ("assignment_date" IS NULL));



COMMENT ON INDEX "public"."job_assignments_whole_job_unique" IS 'Uniqueness for whole-job assignments (assignment_date NULL or single_day=false).';
...
CREATE OR REPLACE TRIGGER "create_timesheets_on_assignment" AFTER INSERT ON "public"."job_assignments" FOR EACH ROW EXECUTE FUNCTION "public"."create_timesheets_for_assignment"();
...
CREATE OR REPLACE TRIGGER "update_equipment_models_updated_at" BEFORE UPDATE ON "public"."equipment_models" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();
...
ALTER TABLE ONLY "public"."house_tech_rates"
    ADD CONSTRAINT "house_tech_rates_profile_id_fkey" FOREIGN KEY ("profile_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;
...
CREATE POLICY "Admin or management can modify skills" ON "public"."skills" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))) WITH CHECK (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Admins and management can modify schedules" ON "public"."push_notification_schedules" TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
...
CREATE POLICY "Admins and management can update vacation requests" ON "public"."vacation_requests" FOR UPDATE USING ((( SELECT "public"."get_current_user_role"() AS "get_current_user_role") = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Admins and management can view all vacation requests" ON "public"."vacation_requests" FOR SELECT USING ((( SELECT "public"."get_current_user_role"() AS "get_current_user_role") = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Admins and management can view schedules" ON "public"."push_notification_schedules" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));



CREATE POLICY "Admins can delete tour assignments" ON "public"."tour_assignments" FOR DELETE USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Admins can manage all profiles" ON "public"."profiles" USING ((( SELECT "public"."get_current_user_role"() AS "get_current_user_role") = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Admins can update any profile" ON "public"."profiles" FOR UPDATE USING (("public"."get_current_user_role"() = 'admin'::"text"));



CREATE POLICY "Admins can update tour assignments" ON "public"."tour_assignments" FOR UPDATE USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))) WITH CHECK (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Admins can view all subscriptions" ON "public"."morning_summary_subscriptions" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = 'admin'::"public"."user_role")))));



CREATE POLICY "Admins can view all tour assignments" ON "public"."tour_assignments" FOR SELECT USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Admins can view push cron logs" ON "public"."push_cron_execution_log" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = 'admin'::"public"."user_role")))));



CREATE POLICY "All users can upload tour documents" ON "public"."tour_documents" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "All users can view job date types" ON "public"."job_date_types" FOR SELECT USING (true);



CREATE POLICY "All users can view job departments" ON "public"."job_departments" FOR SELECT USING (true);



CREATE POLICY "All users can view locations" ON "public"."locations" FOR SELECT USING (true);



CREATE POLICY "All users can view tour dates" ON "public"."tour_dates" FOR SELECT USING (true);



CREATE POLICY "All users can view tour documents" ON "public"."tour_documents" FOR SELECT USING (true);



CREATE POLICY "All users can view tours" ON "public"."tours" FOR SELECT USING (true);



CREATE POLICY "Authenticated can read profile_skills" ON "public"."profile_skills" FOR SELECT USING ((( SELECT "auth"."role"() AS "role") = 'authenticated'::"text"));



CREATE POLICY "Authenticated can read skills" ON "public"."skills" FOR SELECT USING ((( SELECT "auth"."role"() AS "role") = 'authenticated'::"text"));



CREATE POLICY "Authenticated users can upload job documents" ON "public"."job_documents" FOR INSERT WITH CHECK ((("auth"."uid"() IS NOT NULL) AND ("uploaded_by" = "auth"."uid"())));



CREATE POLICY "Authenticated users can view active templates" ON "public"."hoja_de_ruta_templates" FOR SELECT TO "authenticated" USING (("is_active" = true));
...
CREATE POLICY "Authenticated users can view equipment models" ON "public"."equipment_models" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Authenticated users can view flex folders" ON "public"."flex_folders" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Authenticated users can view global presets" ON "public"."global_availability_presets" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Authenticated users can view rate cards" ON "public"."rate_cards_2025" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Authenticated users can view technician availability" ON "public"."technician_availability" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "Department can delete presets" ON "public"."presets" FOR DELETE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "pf"
  WHERE (("pf"."id" = "auth"."uid"()) AND ("pf"."department" = "presets"."department")))));



CREATE POLICY "Department can delete sub_rentals" ON "public"."sub_rentals" FOR DELETE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."department" = "sub_rentals"."department")))));



CREATE POLICY "Department can insert presets" ON "public"."presets" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles" "pf"
  WHERE (("pf"."id" = "auth"."uid"()) AND ("pf"."department" = "presets"."department")))));



CREATE POLICY "Department can insert sub_rentals" ON "public"."sub_rentals" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."department" = "sub_rentals"."department")))));
...
CREATE POLICY "Department can manage preset items" ON "public"."preset_items" USING ((EXISTS ( SELECT 1
   FROM ("public"."presets" "pr"
     JOIN "public"."profiles" "pf" ON (("pf"."id" = "auth"."uid"())))
  WHERE (("pr"."id" = "preset_items"."preset_id") AND ("pf"."department" = "pr"."department"))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."presets" "pr"
     JOIN "public"."profiles" "pf" ON (("pf"."id" = "auth"."uid"())))
...
CREATE POLICY "Department can update presets" ON "public"."presets" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "pf"
  WHERE (("pf"."id" = "auth"."uid"()) AND ("pf"."department" = "presets"."department"))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles" "pf"
  WHERE (("pf"."id" = "auth"."uid"()) AND ("pf"."department" = "presets"."department")))));



CREATE POLICY "Department can update sub_rentals" ON "public"."sub_rentals" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."department" = "sub_rentals"."department"))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."department" = "sub_rentals"."department")))));



CREATE POLICY "Department can view presets" ON "public"."presets" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "pf"
  WHERE (("pf"."id" = "auth"."uid"()) AND ("pf"."department" = "presets"."department")))));



CREATE POLICY "Department can view sub_rentals" ON "public"."sub_rentals" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."department" = "sub_rentals"."department")))));



CREATE POLICY "Department personnel can manage lights personnel" ON "public"."lights_job_personnel" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")) AND ("profiles"."department" = ANY (ARRAY['lights'::"text", 'admin'::"text", 'management'::"text"]))))));



CREATE POLICY "Department personnel can manage sound personnel" ON "public"."sound_job_personnel" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")) AND ("profiles"."department" = ANY (ARRAY['sound'::"text", 'admin'::"text", 'management'::"text"]))))));



CREATE POLICY "Department personnel can manage video personnel" ON "public"."video_job_personnel" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")) AND ("profiles"."department" = ANY (ARRAY['video'::"text", 'admin'::"text", 'management'::"text"]))))));
...
CREATE POLICY "Enable insert for authenticated users only" ON "public"."technician_availability" FOR INSERT TO "authenticated" WITH CHECK (true);
...
CREATE POLICY "Enable read access for all users" ON "public"."tour_default_sets" FOR SELECT USING (true);



CREATE POLICY "Enable read access for all users" ON "public"."tour_default_tables" FOR SELECT USING (true);



CREATE POLICY "Enable read access for all users" ON "public"."tour_power_defaults" FOR SELECT USING (true);



CREATE POLICY "Enable read access for all users" ON "public"."tour_weight_defaults" FOR SELECT USING (true);



CREATE POLICY "House techs can create vacation requests" ON "public"."vacation_requests" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")) AND ("profiles"."role" = ANY (ARRAY['technician'::"public"."user_role", 'house_tech'::"public"."user_role", 'admin'::"public"."user_role", 'management'::"public"."user_role"]))))));



CREATE POLICY "House techs can update their own pending requests" ON "public"."vacation_requests" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")) AND ("profiles"."id" = "vacation_requests"."technician_id") AND (("vacation_requests"."status")::"text" = 'pending'::"text")))));



CREATE POLICY "House techs can view jobs in their departments" ON "public"."jobs" FOR SELECT USING ((("public"."get_current_user_role"() = 'house_tech'::"text") AND (EXISTS ( SELECT 1
   FROM ("public"."profiles" "p"
     JOIN "public"."job_departments" "jd" ON (("jd"."department" = "p"."department")))
  WHERE (("p"."id" = "auth"."uid"()) AND ("jd"."job_id" = "jobs"."id"))))));



CREATE POLICY "Lights personnel can manage lights tasks" ON "public"."lights_job_tasks" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")) AND ("profiles"."department" = ANY (ARRAY['lights'::"text", 'admin'::"text", 'management'::"text"]))))));



CREATE POLICY "Logistics can manage event departments" ON "public"."logistics_event_departments" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text", 'house_tech'::"text"])));



CREATE POLICY "Logistics can manage events" ON "public"."logistics_events" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text", 'house_tech'::"text"])));



CREATE POLICY "Management and authenticated users can view tour rate cards" ON "public"."rate_cards_tour_2025" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role", 'house_tech'::"public"."user_role", 'technician'::"public"."user_role"]))))));



CREATE POLICY "Management can create equipment models" ON "public"."equipment_models" FOR INSERT TO "authenticated" WITH CHECK ("public"."can_manage_users"());



CREATE POLICY "Management can create timesheets" ON "public"."timesheets" FOR INSERT WITH CHECK (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can delete equipment models" ON "public"."equipment_models" FOR DELETE TO "authenticated" USING ("public"."can_manage_users"());
...
CREATE POLICY "Management can do it all" ON "public"."hoja_de_ruta_accommodations" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Management can manage all" ON "public"."hoja_de_ruta_room_assignments" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Management can manage all" ON "public"."hoja_de_ruta_transport" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Management can manage all" ON "public"."hoja_de_ruta_travel_arrangements" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Management can manage all push subscriptions" ON "public"."push_subscriptions" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))) WITH CHECK (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage all timesheets" ON "public"."timesheets" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage assignments" ON "public"."job_assignments" USING ((( SELECT "public"."get_current_user_role"() AS "get_current_user_role") = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage availability" ON "public"."availability_schedules" TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))) WITH CHECK (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));
...
CREATE POLICY "Management can manage department requests" ON "public"."vacation_requests" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "manager_profile"
  WHERE (("manager_profile"."id" = "auth"."uid"()) AND ("manager_profile"."role" = 'management'::"public"."user_role") AND (EXISTS ( SELECT 1
           FROM "public"."profiles" "tech_profile"
          WHERE (("tech_profile"."id" = "vacation_requests"."technician_id") AND ("tech_profile"."department" = "manager_profile"."department"))))))));



CREATE POLICY "Management can manage equipment" ON "public"."equipment" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage festival artist files" ON "public"."festival_artist_files" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage festival assignments" ON "public"."festival_shift_assignments" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage festival data" ON "public"."festival_artists" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage festival form submissions" ON "public"."festival_artist_form_submissions" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage festival forms" ON "public"."festival_artist_forms" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage festival gear" ON "public"."festival_gear_setups" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage festival logos" ON "public"."festival_logos" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage festival settings" ON "public"."festival_settings" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage festival shifts" ON "public"."festival_shifts" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage festival stage gear" ON "public"."festival_stage_gear_setups" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));
...
CREATE POLICY "Management can manage flex folders" ON "public"."flex_folders" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage global presets" ON "public"."global_availability_presets" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));
...
CREATE POLICY "Management can manage hoja templates" ON "public"."hoja_de_ruta_templates" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));
...
CREATE POLICY "Management can manage job date types" ON "public"."job_date_types" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Management can manage job departments" ON "public"."job_departments" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Management can manage job documents" ON "public"."job_documents" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Management can manage jobs" ON "public"."jobs" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Management can manage lights tour tasks" ON "public"."lights_job_tasks" USING ((("tour_id" IS NOT NULL) AND ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"]))));



CREATE POLICY "Management can manage locations" ON "public"."locations" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));
...
CREATE POLICY "Management can manage notification subscriptions" ON "public"."notification_subscriptions" USING ((("user_id" = "auth"."uid"()) OR ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))));



CREATE POLICY "Management can manage rate cards" ON "public"."rate_cards_2025" TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));



CREATE POLICY "Management can manage sound tour tasks" ON "public"."sound_job_tasks" USING ((("tour_id" IS NOT NULL) AND ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"]))));



CREATE POLICY "Management can manage stock" ON "public"."global_stock_entries" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage stock movements" ON "public"."stock_movements" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));
...
CREATE POLICY "Management can manage tour assignments" ON "public"."tour_assignments" USING ((( SELECT "public"."get_current_user_role"() AS "get_current_user_role") = ANY (ARRAY['admin'::"text", 'management'::"text"])));
...
CREATE POLICY "Management can manage tour dates" ON "public"."tour_dates" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage tour default sets" ON "public"."tour_default_sets" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage tour default tables" ON "public"."tour_default_tables" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));
...
CREATE POLICY "Management can manage tour power defaults" ON "public"."tour_power_defaults" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage tour rate cards" ON "public"."rate_cards_tour_2025" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));
...
CREATE POLICY "Management can manage tour weight defaults" ON "public"."tour_weight_defaults" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage tours" ON "public"."tours" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can manage video tour tasks" ON "public"."video_job_tasks" USING ((("tour_id" IS NOT NULL) AND ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"]))));



CREATE POLICY "Management can update equipment models" ON "public"."equipment_models" FOR UPDATE TO "authenticated" USING ("public"."can_manage_users"());



CREATE POLICY "Management can view all assignments" ON "public"."job_assignments" FOR SELECT USING ((( SELECT "public"."get_current_user_role"() AS "get_current_user_role") = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can view all availability" ON "public"."availability_schedules" FOR SELECT USING ((( SELECT "public"."get_current_user_role"() AS "get_current_user_role") = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can view all timesheets" ON "public"."timesheets" FOR SELECT USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Management can view department requests" ON "public"."vacation_requests" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "manager_profile"
  WHERE (("manager_profile"."id" = "auth"."uid"()) AND ("manager_profile"."role" = 'management'::"public"."user_role") AND (EXISTS ( SELECT 1
           FROM "public"."profiles" "tech_profile"
          WHERE (("tech_profile"."id" = "vacation_requests"."technician_id") AND ("tech_profile"."department" = "manager_profile"."department"))))))));
...
CREATE POLICY "Only admins can modify cron config" ON "public"."push_cron_config" TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
...
CREATE POLICY "Only admins can view cron config" ON "public"."push_cron_config" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = 'admin'::"public"."user_role")))));



CREATE POLICY "Only management can delete tour documents" ON "public"."tour_documents" FOR DELETE USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Owner can manage own profile_skills" ON "public"."profile_skills" USING ((("profile_id" = "auth"."uid"()) OR ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])))) WITH CHECK ((("profile_id" = "auth"."uid"()) OR ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))));



CREATE POLICY "Sound personnel can manage sound tasks" ON "public"."sound_job_tasks" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")) AND ("profiles"."department" = ANY (ARRAY['sound'::"text", 'admin'::"text", 'management'::"text"]))))));



CREATE POLICY "Technicians can create own timesheets" ON "public"."timesheets" FOR INSERT WITH CHECK (("technician_id" = "auth"."uid"()));



CREATE POLICY "Technicians can update own assignments" ON "public"."job_assignments" FOR UPDATE USING (("technician_id" = ( SELECT "auth"."uid"() AS "uid")));



CREATE POLICY "Technicians can update own timesheet signatures" ON "public"."timesheets" FOR UPDATE USING ((("technician_id" = "auth"."uid"()) AND ("status" = 'submitted'::"public"."timesheet_status"))) WITH CHECK (("technician_id" = "auth"."uid"()));



CREATE POLICY "Technicians can update own timesheets" ON "public"."timesheets" FOR UPDATE USING (("technician_id" = "auth"."uid"()));



CREATE POLICY "Technicians can view assignments for their jobs" ON "public"."job_assignments" FOR SELECT USING (("job_id" IN ( SELECT "public"."get_user_job_ids"("auth"."uid"()) AS "get_user_job_ids")));



CREATE POLICY "Technicians can view own timesheets" ON "public"."timesheets" FOR SELECT USING (("technician_id" = "auth"."uid"()));



CREATE POLICY "Tour participants can view lights tour tasks" ON "public"."lights_job_tasks" FOR SELECT USING ((("tour_id" IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM "public"."tour_assignments"
  WHERE (("tour_assignments"."tour_id" = "lights_job_tasks"."tour_id") AND ("tour_assignments"."technician_id" = "auth"."uid"()))))));



CREATE POLICY "Tour participants can view sound tour tasks" ON "public"."sound_job_tasks" FOR SELECT USING ((("tour_id" IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM "public"."tour_assignments"
  WHERE (("tour_assignments"."tour_id" = "sound_job_tasks"."tour_id") AND ("tour_assignments"."technician_id" = "auth"."uid"()))))));



CREATE POLICY "Tour participants can view video tour tasks" ON "public"."video_job_tasks" FOR SELECT USING ((("tour_id" IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM "public"."tour_assignments"
  WHERE (("tour_assignments"."tour_id" = "video_job_tasks"."tour_id") AND ("tour_assignments"."technician_id" = "auth"."uid"()))))));



CREATE POLICY "Users can create accommodations for their hojas de ruta" ON "public"."hoja_de_ruta_accommodations" FOR INSERT WITH CHECK (("hoja_de_ruta_id" IN ( SELECT "h"."id"
   FROM "public"."hoja_de_ruta" "h"
  WHERE ("h"."created_by" = "auth"."uid"()))));



CREATE POLICY "Users can create own vacation requests" ON "public"."vacation_requests" FOR INSERT WITH CHECK (("technician_id" = "auth"."uid"()));
...
CREATE POLICY "Users can create restaurants for their hoja de ruta" ON "public"."hoja_de_ruta_restaurants" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
  ...
  WHERE (("hdr"."id" = "hoja_de_ruta_restaurants"."hoja_de_ruta_id") AND (("hdr"."created_by" = "auth"."uid"()) OR ("auth"."uid"() IN ( SELECT "ja"."technician_id"
           FROM "public"."job_assignments" "ja"
          WHERE ("ja"."job_id" = "hdr"."job_id"))))))));



CREATE POLICY "Users can create room assignments for their accommodations" ON "public"."hoja_de_ruta_room_assignments" FOR INSERT WITH CHECK (("accommodation_id" IN ( SELECT "a"."id"
...
CREATE POLICY "Users can create technician availability" ON "public"."technician_availability" FOR INSERT WITH CHECK (("auth"."uid"() IS NOT NULL));



CREATE POLICY "Users can create their own subscriptions" ON "public"."morning_summary_subscriptions" FOR INSERT TO "authenticated" WITH CHECK ((("user_id" = "auth"."uid"()) AND (EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role", 'house_tech'::"public"."user_role"])))))));



CREATE POLICY "Users can create transport for their hojas de ruta" ON "public"."hoja_de_ruta_transport" FOR INSERT WITH CHECK (("hoja_de_ruta_id" IN ( SELECT "h"."id"
   FROM "public"."hoja_de_ruta" "h"
  WHERE ("h"."created_by" = "auth"."uid"()))));
...
CREATE POLICY "Users can create transport request items" ON "public"."transport_request_items" FOR INSERT WITH CHECK (("request_id" IN ( SELECT "transport_requests"."id"
  ...
  WHERE (("transport_requests"."created_by" = "auth"."uid"()) OR ("transport_requests"."job_id" IN ( SELECT "get_user_job_ids"."job_id"
           FROM "public"."get_user_job_ids"("auth"."uid"()) "get_user_job_ids"("job_id"))) OR ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))))));
...
CREATE POLICY "Users can create travel arrangements for their hojas de ruta" ON "public"."hoja_de_ruta_travel_arrangements" FOR INSERT WITH CHECK (("hoja_de_ruta_id" IN ( SELECT "h"."id"
   FROM "public"."hoja_de_ruta" "h"
  WHERE ("h"."created_by" = "auth"."uid"()))));



CREATE POLICY "Users can delete accommodations for their hojas de ruta" ON "public"."hoja_de_ruta_accommodations" FOR DELETE USING (("hoja_de_ruta_id" IN ( SELECT "h"."id"
   FROM "public"."hoja_de_ruta" "h"
  WHERE ("h"."created_by" = "auth"."uid"()))));



CREATE POLICY "Users can delete department or job-related sub_rentals" ON "public"."sub_rentals" FOR DELETE USING (((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = ( SELECT "auth"."uid"() AS "uid")) AND ("p"."department" = "sub_rentals"."department")))) OR (("job_id" IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM "public"."job_assignments" "ja"
  WHERE (("ja"."job_id" = "sub_rentals"."job_id") AND ("ja"."technician_id" = ( SELECT "auth"."uid"() AS "uid"))))))));
...
CREATE POLICY "Users can delete room assignments for their accommodations" ON "public"."hoja_de_ruta_room_assignments" FOR DELETE USING (("accommodation_id" IN ( SELECT "a"."id"
   FROM ("public"."hoja_de_ruta_accommodations" "a"
     JOIN "public"."hoja_de_ruta" "h" ON (("a"."hoja_de_ruta_id" = "h"."id")))
  WHERE ("h"."created_by" = "auth"."uid"()))));
...
CREATE POLICY "Users can delete technician availability" ON "public"."technician_availability" FOR DELETE USING (("auth"."uid"() IS NOT NULL));



CREATE POLICY "Users can delete their own subscriptions" ON "public"."morning_summary_subscriptions" FOR DELETE TO "authenticated" USING (("user_id" = "auth"."uid"()));



CREATE POLICY "Users can delete transport for their hojas de ruta" ON "public"."hoja_de_ruta_transport" FOR DELETE USING (("hoja_de_ruta_id" IN ( SELECT "h"."id"
   FROM "public"."hoja_de_ruta" "h"
  WHERE ("h"."created_by" = "auth"."uid"()))));



CREATE POLICY "Users can delete travel arrangements for their hojas de ruta" ON "public"."hoja_de_ruta_travel_arrangements" FOR DELETE USING (("hoja_de_ruta_id" IN ( SELECT "h"."id"
   FROM "public"."hoja_de_ruta" "h"
  WHERE ("h"."created_by" = "auth"."uid"()))));



CREATE POLICY "Users can insert department or job-related sub_rentals" ON "public"."sub_rentals" FOR INSERT WITH CHECK (((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = ( SELECT "auth"."uid"() AS "uid")) AND ("p"."department" = "sub_rentals"."department")))) OR (("job_id" IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM "public"."job_assignments" "ja"
  WHERE (("ja"."job_id" = "sub_rentals"."job_id") AND ("ja"."technician_id" = ( SELECT "auth"."uid"() AS "uid"))))))));
...
CREATE POLICY "Users can insert own department movements" ON "public"."stock_movements" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."equipment" "e"
  WHERE (("e"."id" = "stock_movements"."equipment_id") AND ("e"."department" = ( SELECT "profiles"."department"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = "auth"."uid"())))))));



CREATE POLICY "Users can insert own department stock" ON "public"."global_stock_entries" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."equipment" "e"
  WHERE (("e"."id" = "global_stock_entries"."equipment_id") AND ("e"."department" = ( SELECT "profiles"."department"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = "auth"."uid"())))))));
...
CREATE POLICY "Users can insert tour assignments" ON "public"."tour_assignments" FOR INSERT WITH CHECK ((( SELECT "public"."get_current_user_role"() AS "get_current_user_role") = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "Users can manage artist files" ON "public"."festival_artist_files" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Users can manage artist forms" ON "public"."festival_artist_forms" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Users can manage day assignments" ON "public"."day_assignments" USING (("user_id" = ( SELECT "auth"."uid"() AS "uid")));



CREATE POLICY "Users can manage direct messages" ON "public"."direct_messages" USING ((("sender_id" = ( SELECT "auth"."uid"() AS "uid")) OR ("recipient_id" = ( SELECT "auth"."uid"() AS "uid"))));



CREATE POLICY "Users can manage festival artists" ON "public"."festival_artists" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Users can manage festival gear setups" ON "public"."festival_gear_setups" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Users can manage festival logos" ON "public"."festival_logos" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Users can manage festival settings" ON "public"."festival_settings" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Users can manage festival shifts" ON "public"."festival_shifts" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Users can manage form submissions" ON "public"."festival_artist_form_submissions" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Users can manage job date types" ON "public"."job_date_types" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));
...
CREATE POLICY "Users can manage own availability exceptions" ON "public"."availability_exceptions" USING (("user_id" = ( SELECT "auth"."uid"() AS "uid")));



CREATE POLICY "Users can manage own messages" ON "public"."messages" USING (("sender_id" = ( SELECT "auth"."uid"() AS "uid")));



CREATE POLICY "Users can manage own notification preferences" ON "public"."notification_preferences" USING (("user_id" = ( SELECT "auth"."uid"() AS "uid")));



CREATE POLICY "Users can manage own notifications" ON "public"."assignment_notifications" USING (("technician_id" = ( SELECT "auth"."uid"() AS "uid")));



CREATE POLICY "Users can manage own work records" ON "public"."technician_work_records" USING ((("technician_id" = "auth"."uid"()) OR ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))));
...
CREATE POLICY "Users can manage preset items" ON "public"."preset_items" USING ((EXISTS ( SELECT 1
   FROM "public"."presets"
  WHERE (("presets"."id" = "preset_items"."preset_id") AND ("presets"."user_id" = ( SELECT "auth"."uid"() AS "uid"))))));



CREATE POLICY "Users can manage shift assignments" ON "public"."festival_shift_assignments" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Users can manage stage gear setups" ON "public"."festival_stage_gear_setups" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));



CREATE POLICY "Users can manage task documents" ON "public"."task_documents" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "Users can update accommodations for their hojas de ruta" ON "public"."hoja_de_ruta_accommodations" FOR UPDATE USING (("hoja_de_ruta_id" IN ( SELECT "h"."id"
   FROM "public"."hoja_de_ruta" "h"
  WHERE ("h"."created_by" = "auth"."uid"()))));



CREATE POLICY "Users can update department or job-related sub_rentals" ON "public"."sub_rentals" FOR UPDATE USING (((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = ( SELECT "auth"."uid"() AS "uid")) AND ("p"."department" = "sub_rentals"."department")))) OR (("job_id" IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM "public"."job_assignments" "ja"
  WHERE (("ja"."job_id" = "sub_rentals"."job_id") AND ("ja"."technician_id" = ( SELECT "auth"."uid"() AS "uid"))))))));
...
CREATE POLICY "Users can update own department stock" ON "public"."global_stock_entries" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."equipment" "e"
  WHERE (("e"."id" = "global_stock_entries"."equipment_id") AND ("e"."department" = ( SELECT "profiles"."department"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = "auth"."uid"())))))));
...
CREATE POLICY "Users can update own pending requests" ON "public"."vacation_requests" FOR UPDATE USING ((("technician_id" = "auth"."uid"()) AND (("status")::"text" = 'pending'::"text")));



CREATE POLICY "Users can update own push subscriptions" ON "public"."push_subscriptions" FOR UPDATE USING (("user_id" = "auth"."uid"())) WITH CHECK (("user_id" = "auth"."uid"()));



CREATE POLICY "Users can update own transport requests" ON "public"."transport_requests" FOR UPDATE USING ((("created_by" = "auth"."uid"()) OR ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])))) WITH CHECK ((("created_by" = "auth"."uid"()) OR ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))));
...
CREATE POLICY "Users can update room assignments for their accommodations" ON "public"."hoja_de_ruta_room_assignments" FOR UPDATE USING (("accommodation_id" IN ( SELECT "a"."id"
   FROM ("public"."hoja_de_ruta_accommodations" "a"
     JOIN "public"."hoja_de_ruta" "h" ON (("a"."hoja_de_ruta_id" = "h"."id")))
  WHERE ("h"."created_by" = "auth"."uid"()))));
...
CREATE POLICY "Users can update technician availability" ON "public"."technician_availability" FOR UPDATE USING (("auth"."uid"() IS NOT NULL));



CREATE POLICY "Users can update their own subscriptions" ON "public"."morning_summary_subscriptions" FOR UPDATE TO "authenticated" USING (("user_id" = "auth"."uid"())) WITH CHECK ((("user_id" = "auth"."uid"()) AND (EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role", 'house_tech'::"public"."user_role"])))))));



CREATE POLICY "Users can update transport for their hojas de ruta" ON "public"."hoja_de_ruta_transport" FOR UPDATE USING (("hoja_de_ruta_id" IN ( SELECT "h"."id"
   FROM "public"."hoja_de_ruta" "h"
  WHERE ("h"."created_by" = "auth"."uid"()))));
...
CREATE POLICY "Users can update transport request items" ON "public"."transport_request_items" FOR UPDATE USING (("request_id" IN ( SELECT "transport_requests"."id"
   FROM "public"."transport_requests"
  WHERE (("transport_requests"."created_by" = "auth"."uid"()) OR ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])))))) WITH CHECK (("request_id" IN ( SELECT "transport_requests"."id"
   FROM "public"."transport_requests"
  WHERE (("transport_requests"."created_by" = "auth"."uid"()) OR ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))))));



CREATE POLICY "Users can update travel arrangements for their hojas de ruta" ON "public"."hoja_de_ruta_travel_arrangements" FOR UPDATE USING (("hoja_de_ruta_id" IN ( SELECT "h"."id"
   FROM "public"."hoja_de_ruta" "h"
  WHERE ("h"."created_by" = "auth"."uid"()))));
...
CREATE POLICY "Users can view accessible transport requests" ON "public"."transport_requests" FOR SELECT USING ((("created_by" = "auth"."uid"()) OR ("job_id" IN ( SELECT "get_user_job_ids"."job_id"
   FROM "public"."get_user_job_ids"("auth"."uid"()) "get_user_job_ids"("job_id"))) OR ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))));



CREATE POLICY "Users can view accommodations from their hojas de ruta" ON "public"."hoja_de_ruta_accommodations" FOR SELECT USING (("hoja_de_ruta_id" IN ( SELECT "h"."id"
   FROM "public"."hoja_de_ruta" "h"
  WHERE (("h"."created_by" = "auth"."uid"()) OR ("h"."approved_by" = "auth"."uid"())))));



CREATE POLICY "Users can view artist files" ON "public"."festival_artist_files" FOR SELECT USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text", 'technician'::"text", 'house_tech'::"text"])));



CREATE POLICY "Users can view artist forms" ON "public"."festival_artist_forms" FOR SELECT USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text", 'technician'::"text", 'house_tech'::"text"])));



CREATE POLICY "Users can view department messages" ON "public"."messages" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")) AND (("profiles"."department" = "messages"."department") OR ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])))))));



CREATE POLICY "Users can view department or job-related sub_rentals" ON "public"."sub_rentals" FOR SELECT USING (((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = ( SELECT "auth"."uid"() AS "uid")) AND ("p"."department" = "sub_rentals"."department")))) OR (("job_id" IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM "public"."job_assignments" "ja"
  WHERE (("ja"."job_id" = "sub_rentals"."job_id") AND ("ja"."technician_id" = ( SELECT "auth"."uid"() AS "uid"))))))));
...
CREATE POLICY "Users can view festival artists" ON "public"."festival_artists" FOR SELECT USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text", 'technician'::"text", 'house_tech'::"text"])));



CREATE POLICY "Users can view festival gear setups" ON "public"."festival_gear_setups" FOR SELECT USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text", 'technician'::"text", 'house_tech'::"text"])));



CREATE POLICY "Users can view festival logos" ON "public"."festival_logos" FOR SELECT USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text", 'technician'::"text", 'house_tech'::"text"])));



CREATE POLICY "Users can view festival settings" ON "public"."festival_settings" FOR SELECT USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text", 'technician'::"text", 'house_tech'::"text"])));



CREATE POLICY "Users can view festival shifts" ON "public"."festival_shifts" FOR SELECT USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text", 'technician'::"text", 'house_tech'::"text"])));



CREATE POLICY "Users can view form submissions" ON "public"."festival_artist_form_submissions" FOR SELECT USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text", 'technician'::"text", 'house_tech'::"text"])));



CREATE POLICY "Users can view job date types" ON "public"."job_date_types" FOR SELECT USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text", 'technician'::"text", 'house_tech'::"text"])));



CREATE POLICY "Users can view jobs they are assigned to" ON "public"."jobs" FOR SELECT USING ((("id" IN ( SELECT "job_assignments"."job_id"
   FROM "public"."job_assignments"
  WHERE ("job_assignments"."technician_id" = "auth"."uid"()))) OR ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"]))));



CREATE POLICY "Users can view own department equipment" ON "public"."equipment" FOR SELECT TO "authenticated" USING (("department" = ( SELECT "profiles"."department"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = "auth"."uid"()))));



CREATE POLICY "Users can view own department movements" ON "public"."stock_movements" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."equipment" "e"
  WHERE (("e"."id" = "stock_movements"."equipment_id") AND ("e"."department" = ( SELECT "profiles"."department"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = "auth"."uid"())))))));
...
CREATE POLICY "Users can view own department stock" ON "public"."global_stock_entries" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."equipment" "e"
  WHERE (("e"."id" = "global_stock_entries"."equipment_id") AND ("e"."department" = ( SELECT "profiles"."department"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = "auth"."uid"())))))));
...
CREATE POLICY "Users can view own vacation requests" ON "public"."vacation_requests" FOR SELECT USING (("technician_id" = "auth"."uid"()));
...
CREATE POLICY "Users can view room assignments from their accommodations" ON "public"."hoja_de_ruta_room_assignments" FOR SELECT USING (("accommodation_id" IN ( SELECT "a"."id"
   FROM ("public"."hoja_de_ruta_accommodations" "a"
     JOIN "public"."hoja_de_ruta" "h" ON (("a"."hoja_de_ruta_id" = "h"."id")))
  WHERE (("h"."created_by" = "auth"."uid"()) OR ("h"."approved_by" = "auth"."uid"())))));
...
CREATE POLICY "Users can view shift assignments" ON "public"."festival_shift_assignments" FOR SELECT USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text", 'technician'::"text", 'house_tech'::"text"])));



CREATE POLICY "Users can view stage gear setups" ON "public"."festival_stage_gear_setups" FOR SELECT USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text", 'technician'::"text", 'house_tech'::"text"])));



CREATE POLICY "Users can view their own subscriptions" ON "public"."morning_summary_subscriptions" FOR SELECT TO "authenticated" USING (("user_id" = "auth"."uid"()));



CREATE POLICY "Users can view their own tour assignments" ON "public"."tour_assignments" FOR SELECT USING (("technician_id" = ( SELECT "auth"."uid"() AS "uid")));



CREATE POLICY "Users can view their own vacation requests" ON "public"."vacation_requests" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")) AND ("profiles"."id" = "vacation_requests"."technician_id")))));



CREATE POLICY "Users can view transport from their hojas de ruta" ON "public"."hoja_de_ruta_transport" FOR SELECT USING (("hoja_de_ruta_id" IN ( SELECT "h"."id"
   FROM "public"."hoja_de_ruta" "h"
  WHERE (("h"."created_by" = "auth"."uid"()) OR ("h"."approved_by" = "auth"."uid"())))));



CREATE POLICY "Users can view travel arrangements from their hojas de ruta" ON "public"."hoja_de_ruta_travel_arrangements" FOR SELECT USING (("hoja_de_ruta_id" IN ( SELECT "h"."id"
   FROM "public"."hoja_de_ruta" "h"
  WHERE (("h"."created_by" = "auth"."uid"()) OR ("h"."approved_by" = "auth"."uid"())))));



CREATE POLICY "Users can view visible timesheet amounts" ON "public"."timesheets" FOR SELECT USING ((("auth"."uid"() = "technician_id") OR (EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])))))));



CREATE POLICY "Video personnel can manage video tasks" ON "public"."video_job_tasks" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")) AND ("profiles"."department" = ANY (ARRAY['video'::"text", 'admin'::"text", 'management'::"text"]))))));
...
CREATE POLICY "activity_log_actor_only" ON "public"."activity_log" FOR SELECT USING ((("visibility" = 'actor_only'::"public"."activity_visibility") AND ("actor_id" = "auth"."uid"())));



CREATE POLICY "activity_log_house_plus" ON "public"."activity_log" FOR SELECT USING ((("visibility" = 'house_plus_job'::"public"."activity_visibility") AND (EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = 'house_tech'::"public"."user_role"))))));



CREATE POLICY "activity_log_job_participants" ON "public"."activity_log" FOR SELECT USING ((("visibility" = 'job_participants'::"public"."activity_visibility") AND ("job_id" IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM "public"."job_assignments" "ja"
  WHERE (("ja"."job_id" = "activity_log"."job_id") AND ("ja"."technician_id" = "auth"."uid"()))))));



CREATE POLICY "activity_log_management" ON "public"."activity_log" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['management'::"public"."user_role", 'admin'::"public"."user_role"]))))));
...
CREATE POLICY "activity_prefs_owner" ON "public"."activity_prefs" USING (("user_id" = "auth"."uid"())) WITH CHECK (("user_id" = "auth"."uid"()));
...
CREATE POLICY "activity_reads_owner" ON "public"."activity_reads" USING (("user_id" = "auth"."uid"())) WITH CHECK (("user_id" = "auth"."uid"()));



CREATE POLICY "all mgmt" ON "public"."push_notification_routes" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))) WITH CHECK (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));
...
CREATE POLICY "app_changelog_delete_admin" ON "public"."app_changelog" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = 'admin'::"public"."user_role")))));



CREATE POLICY "app_changelog_insert_editors" ON "public"."app_changelog" FOR INSERT TO "authenticated" WITH CHECK (((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['management'::"public"."user_role", 'admin'::"public"."user_role"]))))) OR ("lower"(COALESCE(("auth"."jwt"() ->> 'email'::"text"), ''::"text")) = 'sonido@sector-pro.com'::"text")));
...
CREATE POLICY "app_changelog_update_editors" ON "public"."app_changelog" FOR UPDATE TO "authenticated" USING (((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['management'::"public"."user_role", 'admin'::"public"."user_role"]))))) OR ("lower"(COALESCE(("auth"."jwt"() ->> 'email'::"text"), ''::"text")) = 'sonido@sector-pro.com'::"text"))) WITH CHECK (((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['management'::"public"."user_role", 'admin'::"public"."user_role"]))))) OR ("lower"(COALESCE(("auth"."jwt"() ->> 'email'::"text"), ''::"text")) = 'sonido@sector-pro.com'::"text")));
...
CREATE POLICY "del mgmt" ON "public"."push_notification_routes" FOR DELETE USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));
...
CREATE POLICY "dm_delete_self" ON "public"."direct_messages" FOR DELETE USING ((("auth"."uid"() = "sender_id") OR ("auth"."uid"() = "recipient_id")));



CREATE POLICY "dm_insert_authenticated" ON "public"."direct_messages" FOR INSERT WITH CHECK ((("auth"."uid"() = "sender_id") OR ("auth"."uid"() = "recipient_id")));



CREATE POLICY "dm_select_self" ON "public"."direct_messages" FOR SELECT USING ((("auth"."uid"() = "sender_id") OR ("auth"."uid"() = "recipient_id")));



CREATE POLICY "dm_update_status_recipient" ON "public"."direct_messages" FOR UPDATE USING (("auth"."uid"() = "recipient_id")) WITH CHECK (("auth"."uid"() = "recipient_id"));



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."activity_catalog" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."activity_log" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."activity_prefs" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."activity_reads" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."announcements" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."app_changelog" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."assignment_notifications" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."availability_conflicts" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."availability_exceptions" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."availability_schedules" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."day_assignments" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."day_preset_assignments" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."direct_messages" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."dwg_conversion_queue" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."equipment" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."equipment_models" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."festival_artist_files" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."festival_artist_form_submissions" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."festival_artist_forms" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."festival_artists" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."festival_gear_setups" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."festival_logos" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."festival_settings" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."festival_shift_assignments" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."festival_shifts" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."festival_stage_gear_setups" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."festival_stages" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."flex_crew_assignments" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."flex_crew_calls" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."flex_folders" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."flex_status_log" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."flex_work_order_items" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."flex_work_orders" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."global_availability_presets" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."global_stock_entries" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."hoja_de_ruta" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."hoja_de_ruta_accommodations" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."hoja_de_ruta_contacts" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."hoja_de_ruta_equipment" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."hoja_de_ruta_images" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."hoja_de_ruta_logistics" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."hoja_de_ruta_restaurants" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."hoja_de_ruta_room_assignments" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."hoja_de_ruta_rooms" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."hoja_de_ruta_staff" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."hoja_de_ruta_templates" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."hoja_de_ruta_transport" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."hoja_de_ruta_travel" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."hoja_de_ruta_travel_arrangements" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."house_tech_rates" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."job_assignments" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."job_date_types" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."job_departments" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."job_documents" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."job_milestone_definitions" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."job_milestones" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."job_rate_extras" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."job_required_roles" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."job_whatsapp_group_requests" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."job_whatsapp_groups" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."jobs" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."lights_job_personnel" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."lights_job_tasks" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."lights_memoria_tecnica_documents" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."locations" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."logistics_event_departments" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."logistics_events" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."memoria_tecnica_documents" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."messages" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."milestone_definitions" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."notification_preferences" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."notification_subscriptions" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."power_requirement_tables" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."preset_items" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."presets" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."profile_skills" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."profiles" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."push_subscriptions" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."rate_cards_2025" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."rate_cards_tour_2025" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."rate_extras_2025" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."required_docs" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."secrets" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."skills" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."sound_job_personnel" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."sound_job_tasks" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."soundvision_file_reviews" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."soundvision_files" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."staffing_events" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."staffing_requests" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."stock_movements" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."sub_rentals" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."task_documents" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."technician_availability" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."technician_departments" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."technician_fridge" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."technician_work_records" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."timesheets" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."tour_assignments" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."tour_date_power_overrides" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."tour_date_weight_overrides" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."tour_dates" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."tour_default_sets" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."tour_default_tables" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."tour_documents" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."tour_logos" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."tour_power_defaults" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."tour_week_multipliers_2025" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."tour_weight_defaults" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."tours" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."transport_request_items" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."transport_requests" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."vacation_requests" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."venues" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."video_job_personnel" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."video_job_tasks" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."video_memoria_tecnica_documents" FOR SELECT TO "dreamlit_app" USING (true);



CREATE POLICY "dreamlit_dreamlit_app_select_policy" ON "public"."wallboard_presets" FOR SELECT TO "dreamlit_app" USING (true);
...
CREATE POLICY "festival_artists_select_all" ON "public"."festival_artists" FOR SELECT USING (true);



CREATE POLICY "festival_artists_select_job_participants" ON "public"."festival_artists" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."job_assignments" "ja"
  WHERE (("ja"."job_id" = "festival_artists"."job_id") AND ("ja"."technician_id" = "auth"."uid"())))));



CREATE POLICY "festival_artists_select_management" ON "public"."festival_artists" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));
...
CREATE POLICY "festival_riders_select_all" ON "public"."festival_artist_files" FOR SELECT USING (true);



CREATE POLICY "festival_riders_select_job_participants" ON "public"."festival_artist_files" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM ("public"."festival_artists" "fa"
     JOIN "public"."job_assignments" "ja" ON (("ja"."job_id" = "fa"."job_id")))
  WHERE (("fa"."id" = "festival_artist_files"."artist_id") AND ("ja"."technician_id" = "auth"."uid"())))));



CREATE POLICY "festival_riders_select_management" ON "public"."festival_artist_files" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));
...
CREATE POLICY "house_rates_mgr_ins" ON "public"."house_tech_rates" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));



CREATE POLICY "house_rates_mgr_sel" ON "public"."house_tech_rates" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));



CREATE POLICY "house_rates_mgr_upd" ON "public"."house_tech_rates" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));



CREATE POLICY "house_tech_calendar_availability" ON "public"."availability_schedules" FOR SELECT USING ((("public"."get_current_user_role"() = ANY (ARRAY['house_tech'::"text", 'admin'::"text", 'management'::"text", 'logistics'::"text"])) AND ("user_id" IN ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."role" = 'house_tech'::"public"."user_role")))));



CREATE POLICY "house_tech_calendar_job_assignments" ON "public"."job_assignments" FOR SELECT USING ((("public"."get_current_user_role"() = ANY (ARRAY['house_tech'::"text", 'admin'::"text", 'management'::"text", 'logistics'::"text"])) AND ("technician_id" IN ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."role" = 'house_tech'::"public"."user_role")))));



CREATE POLICY "house_tech_calendar_jobs" ON "public"."jobs" FOR SELECT USING ((("public"."get_current_user_role"() = ANY (ARRAY['house_tech'::"text", 'admin'::"text", 'management'::"text", 'logistics'::"text"])) AND (EXISTS ( SELECT 1
   FROM ("public"."job_assignments" "ja"
     JOIN "public"."profiles" "tech" ON (("tech"."id" = "ja"."technician_id")))
  WHERE (("ja"."job_id" = "jobs"."id") AND ("tech"."role" = 'house_tech'::"public"."user_role"))))));



CREATE POLICY "house_tech_calendar_locations" ON "public"."locations" FOR SELECT USING ((("public"."get_current_user_role"() = ANY (ARRAY['house_tech'::"text", 'admin'::"text", 'management'::"text", 'logistics'::"text"])) AND (EXISTS ( SELECT 1
   FROM (("public"."jobs" "j"
     JOIN "public"."job_assignments" "ja" ON (("ja"."job_id" = "j"."id")))
     JOIN "public"."profiles" "tech" ON (("tech"."id" = "ja"."technician_id")))
  WHERE (("j"."location_id" = "locations"."id") AND ("tech"."role" = 'house_tech'::"public"."user_role"))))));



CREATE POLICY "house_tech_calendar_profiles" ON "public"."profiles" FOR SELECT USING ((("public"."get_current_user_role"() = ANY (ARRAY['house_tech'::"text", 'admin'::"text", 'management'::"text", 'logistics'::"text"])) AND ("role" = 'house_tech'::"public"."user_role")));



ALTER TABLE "public"."house_tech_rates" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "insert mgmt" ON "public"."push_notification_routes" FOR INSERT WITH CHECK (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));
...
CREATE POLICY "job_extras_mgr_read" ON "public"."job_rate_extras" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));



CREATE POLICY "job_extras_mgr_write" ON "public"."job_rate_extras" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));
...
CREATE POLICY "job_extras_tech_read" ON "public"."job_rate_extras" FOR SELECT USING (("technician_id" = "auth"."uid"()));
...
CREATE POLICY "job_required_roles_manage" ON "public"."job_required_roles" TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))) WITH CHECK (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "job_required_roles_select" ON "public"."job_required_roles" FOR SELECT TO "authenticated" USING ((("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'coordinator'::"text", 'logistics'::"text"])) OR (EXISTS ( SELECT 1
   FROM "public"."job_assignments" "ja"
  WHERE (("ja"."job_id" = "job_required_roles"."job_id") AND ("ja"."technician_id" = "auth"."uid"()))))));
...
CREATE POLICY "job_whatsapp_group_requests_no_mod_client" ON "public"."job_whatsapp_group_requests" TO "authenticated" USING (false) WITH CHECK (false);



CREATE POLICY "job_whatsapp_group_requests_select_mgmt" ON "public"."job_whatsapp_group_requests" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));
...
CREATE POLICY "job_whatsapp_groups_no_mod_client" ON "public"."job_whatsapp_groups" TO "authenticated" USING (false) WITH CHECK (false);



CREATE POLICY "job_whatsapp_groups_select_mgmt" ON "public"."job_whatsapp_groups" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));
...
CREATE POLICY "management_update_job_rates_approval" ON "public"."jobs" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));
...
CREATE POLICY "management_update_tour_rates_approval" ON "public"."tours" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));
...
CREATE POLICY "messages_delete_self_or_mgmt" ON "public"."messages" FOR DELETE USING ((("sender_id" = "auth"."uid"()) OR (EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])) AND ("p"."department" = "messages"."department"))))));



CREATE POLICY "messages_select_self_or_mgmt" ON "public"."messages" FOR SELECT USING ((("sender_id" = "auth"."uid"()) OR (EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])) AND ("p"."department" = "messages"."department"))))));



CREATE POLICY "messages_update_status_mgmt" ON "public"."messages" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])) AND ("p"."department" = "messages"."department"))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])) AND ("p"."department" = "messages"."department")))));



CREATE POLICY "mgmt_can_update_job_documents" ON "public"."job_documents" FOR UPDATE TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))) WITH CHECK (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "mgmt_can_view_all_job_documents" ON "public"."job_documents" FOR SELECT TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'logistics'::"text"])));
...
CREATE POLICY "pm_manage_staffing" ON "public"."staffing_requests" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))) WITH CHECK (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));
...
CREATE POLICY "profiles_select_management" ON "public"."profiles" FOR SELECT USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));
...
CREATE POLICY "profiles_update_management" ON "public"."profiles" FOR UPDATE USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))) WITH CHECK (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));
...
CREATE POLICY "rate_extras_2025_mgr_write" ON "public"."rate_extras_2025" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
  WHERE (("p"."id" = "auth"."uid"()) AND ("p"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));
...
CREATE POLICY "rate_extras_2025_read" ON "public"."rate_extras_2025" FOR SELECT USING (true);
...
CREATE POLICY "select mgmt" ON "public"."push_notification_routes" FOR SELECT USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));
...
CREATE POLICY "soundvision_file_reviews_delete_self_or_management" ON "public"."soundvision_file_reviews" FOR DELETE TO "authenticated" USING ((("reviewer_id" = "auth"."uid"()) OR "public"."is_management_or_admin"("auth"."uid"())));



CREATE POLICY "soundvision_file_reviews_insert_self_or_management" ON "public"."soundvision_file_reviews" FOR INSERT TO "authenticated" WITH CHECK ((("reviewer_id" = "auth"."uid"()) OR "public"."is_management_or_admin"("auth"."uid"())));
...
CREATE POLICY "soundvision_file_reviews_update_self_or_management" ON "public"."soundvision_file_reviews" FOR UPDATE TO "authenticated" USING ((("reviewer_id" = "auth"."uid"()) OR "public"."is_management_or_admin"("auth"."uid"()))) WITH CHECK ((("reviewer_id" = "auth"."uid"()) OR "public"."is_management_or_admin"("auth"."uid"())));
...
CREATE POLICY "soundvision_files_delete_management" ON "public"."soundvision_files" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));



CREATE POLICY "soundvision_files_insert_authorized" ON "public"."soundvision_files" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND (("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])) OR COALESCE("profiles"."soundvision_access_enabled", false) OR (("profiles"."role" = 'house_tech'::"public"."user_role") AND ("profiles"."department" = 'sound'::"text")))))));



CREATE POLICY "soundvision_files_select_authenticated" ON "public"."soundvision_files" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND (("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])) OR COALESCE("profiles"."soundvision_access_enabled", false) OR (("profiles"."role" = 'house_tech'::"public"."user_role") AND ("profiles"."department" = 'sound'::"text")))))));



CREATE POLICY "soundvision_files_update_authorized" ON "public"."soundvision_files" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND (("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])) OR COALESCE("profiles"."soundvision_access_enabled", false) OR (("profiles"."role" = 'house_tech'::"public"."user_role") AND ("profiles"."department" = 'sound'::"text"))))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND (("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])) OR COALESCE("profiles"."soundvision_access_enabled", false) OR (("profiles"."role" = 'house_tech'::"public"."user_role") AND ("profiles"."department" = 'sound'::"text")))))));
...
CREATE POLICY "tech_read_own_staffing" ON "public"."staffing_requests" FOR SELECT USING (("profile_id" = "auth"."uid"()));
...
CREATE POLICY "technician_fridge_modify_management" ON "public"."technician_fridge" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))) WITH CHECK (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "technician_fridge_select_all" ON "public"."technician_fridge" FOR SELECT USING (("auth"."role"() = 'authenticated'::"text"));
...
CREATE POLICY "technicians_view_assigned_tours" ON "public"."tours" FOR SELECT TO "authenticated" USING (((EXISTS ( SELECT 1
   FROM "public"."tour_assignments" "ta"
  WHERE (("ta"."tour_id" = "tours"."id") AND ("ta"."technician_id" = "auth"."uid"())))) OR (EXISTS ( SELECT 1
   FROM (("public"."job_assignments" "ja"
     JOIN "public"."jobs" "j" ON (("j"."id" = "ja"."job_id")))
     LEFT JOIN "public"."tour_dates" "td" ON (("td"."id" = "j"."tour_date_id")))
  WHERE (("ja"."technician_id" = "auth"."uid"()) AND (("j"."tour_id" = "tours"."id") OR ("td"."tour_id" = "tours"."id")))))));



CREATE POLICY "techs_can_view_visible_docs_for_assigned_jobs" ON "public"."job_documents" FOR SELECT TO "authenticated" USING ((("visible_to_tech" = true) AND ("job_id" IN ( SELECT "ja"."job_id"
   FROM "public"."job_assignments" "ja"
...
CREATE POLICY "tour_accommodations_management" ON "public"."tour_accommodations" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));
...
CREATE POLICY "tour_accommodations_select" ON "public"."tour_accommodations" FOR SELECT USING (((EXISTS ( SELECT 1
   FROM "public"."tour_assignments"
  WHERE (("tour_assignments"."tour_id" = "tour_accommodations"."tour_id") AND ("tour_assignments"."technician_id" = "auth"."uid"())))) OR (EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])))))));
...
CREATE POLICY "tour_schedule_templates_management" ON "public"."tour_schedule_templates" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));
...
CREATE POLICY "tour_schedule_templates_select" ON "public"."tour_schedule_templates" FOR SELECT USING ((("is_global" = true) OR ("tour_id" IS NULL) OR (EXISTS ( SELECT 1
   FROM "public"."tour_assignments"
  WHERE (("tour_assignments"."tour_id" = "tour_schedule_templates"."tour_id") AND ("tour_assignments"."technician_id" = "auth"."uid"())))) OR (EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])))))));
...
CREATE POLICY "tour_timeline_events_management" ON "public"."tour_timeline_events" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));
...
CREATE POLICY "tour_timeline_events_select" ON "public"."tour_timeline_events" FOR SELECT USING (((EXISTS ( SELECT 1
   FROM "public"."tour_assignments"
  WHERE (("tour_assignments"."tour_id" = "tour_timeline_events"."tour_id") AND ("tour_assignments"."technician_id" = "auth"."uid"())))) OR (EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])))))));
...
CREATE POLICY "tour_travel_segments_management" ON "public"."tour_travel_segments" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));
...
CREATE POLICY "tour_travel_segments_select" ON "public"."tour_travel_segments" FOR SELECT USING (((EXISTS ( SELECT 1
   FROM "public"."tour_assignments"
  WHERE (("tour_assignments"."tour_id" = "tour_travel_segments"."tour_id") AND ("tour_assignments"."technician_id" = "auth"."uid"())))) OR (EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"])))))));
...
CREATE POLICY "update mgmt" ON "public"."push_notification_routes" FOR UPDATE USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))) WITH CHECK (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));
...
CREATE POLICY "venues_delete_management" ON "public"."venues" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role"]))))));



CREATE POLICY "venues_insert_authorized" ON "public"."venues" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role", 'house_tech'::"public"."user_role", 'technician'::"public"."user_role", 'logistics'::"public"."user_role"]))))));
...
CREATE POLICY "venues_update_authorized" ON "public"."venues" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = ANY (ARRAY['admin'::"public"."user_role", 'management'::"public"."user_role", 'house_tech'::"public"."user_role", 'technician'::"public"."user_role", 'logistics'::"public"."user_role"]))))));
...
CREATE POLICY "wb_assign_select" ON "public"."job_assignments" FOR SELECT TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'wallboard'::"text"])));



CREATE POLICY "wb_job_departments_select" ON "public"."job_departments" FOR SELECT TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'wallboard'::"text", 'house_tech'::"text"])));



CREATE POLICY "wb_job_documents_select" ON "public"."job_documents" FOR SELECT TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'wallboard'::"text", 'house_tech'::"text"])));



CREATE POLICY "wb_jobs_select" ON "public"."jobs" FOR SELECT TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'wallboard'::"text"])));



CREATE POLICY "wb_locations_select" ON "public"."locations" FOR SELECT TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'wallboard'::"text", 'house_tech'::"text"])));



CREATE POLICY "wb_logistics_event_depts_select" ON "public"."logistics_event_departments" FOR SELECT TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'wallboard'::"text"])));



CREATE POLICY "wb_logistics_events_select" ON "public"."logistics_events" FOR SELECT TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'wallboard'::"text"])));



CREATE POLICY "wb_presets_admin_write" ON "public"."wallboard_presets" TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"]))) WITH CHECK (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text"])));



CREATE POLICY "wb_presets_select" ON "public"."wallboard_presets" FOR SELECT TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'wallboard'::"text"])));



CREATE POLICY "wb_profiles_select" ON "public"."profiles" FOR SELECT TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'wallboard'::"text", 'house_tech'::"text"])));
...
CREATE POLICY "wb_timesheets_select" ON "public"."timesheets" FOR SELECT TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'wallboard'::"text"])));



CREATE POLICY "wb_tours_select" ON "public"."tours" FOR SELECT TO "authenticated" USING (("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'management'::"text", 'wallboard'::"text"])));
...
GRANT ALL ON SCHEMA "dreamlit" TO "dreamlit_app";
...
GRANT ALL ON SCHEMA "public" TO "dreamlit_app";



GRANT ALL ON SCHEMA "secrets" TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "dreamlit"."send_supabase_auth_email"("event" "jsonb") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."assert_soundvision_access"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."attach_soundvision_template"() TO "dreamlit_app";



GRANT ALL ON FUNCTION "public"."auto_complete_past_jobs"() TO "anon";
...
GRANT ALL ON FUNCTION "public"."auto_complete_past_jobs"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."can_manage_users"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."check_technician_conflicts"("_technician_id" "uuid", "_target_job_id" "uuid", "_target_date" "date", "_single_day" boolean, "_include_pending" boolean) TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."cleanup_tour_assignments_from_jobs"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."clear_tour_preset_assignments"("_preset_id" "uuid", "_tour_id" "uuid") TO "dreamlit_app";



GRANT ALL ON FUNCTION "public"."compute_timesheet_amount_2025"("_timesheet_id" "uuid", "_persist" boolean) TO "anon";
...
GRANT ALL ON FUNCTION "public"."compute_timesheet_amount_2025"("_timesheet_id" "uuid", "_persist" boolean) TO "dreamlit_app";



GRANT ALL ON FUNCTION "public"."compute_tour_job_rate_quote_2025"("_job_id" "uuid", "_tech_id" "uuid") TO "anon";
...
GRANT ALL ON FUNCTION "public"."compute_tour_job_rate_quote_2025"("_job_id" "uuid", "_tech_id" "uuid") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."convert_to_timezone"("timestamp_val" timestamp with time zone, "target_timezone" "text") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."create_default_logistics_events"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."create_default_logistics_events_for_job"("job_id" "uuid") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."create_timesheets_for_assignment"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."delete_timesheets_on_assignment_removal"() TO "dreamlit_app";



GRANT ALL ON FUNCTION "public"."dreamlit_auth_admin_executor"("command" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."dreamlit_auth_admin_executor"("command" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."dreamlit_auth_admin_executor"("command" "text") TO "service_role";
GRANT ALL ON FUNCTION "public"."dreamlit_auth_admin_executor"("command" "text") TO "dreamlit_app";



GRANT ALL ON FUNCTION "public"."extras_total_for_job_tech"("_job_id" "uuid", "_tech_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."extras_total_for_job_tech"("_job_id" "uuid", "_tech_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."extras_total_for_job_tech"("_job_id" "uuid", "_tech_id" "uuid") TO "service_role";
GRANT ALL ON FUNCTION "public"."extras_total_for_job_tech"("_job_id" "uuid", "_tech_id" "uuid") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."fn_app_changelog_touch"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."get_assignment_matrix_staffing"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."get_current_user_role"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."get_job_total_amounts"("_job_id" "uuid", "_user_role" "text") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."get_profiles_with_skills"() TO "dreamlit_app";



GRANT ALL ON FUNCTION "public"."get_timesheet_amounts_visible"() TO "anon";
...
GRANT ALL ON FUNCTION "public"."get_timesheet_amounts_visible"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."get_timesheet_effective_rate"("_timesheet_id" "uuid") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."get_timesheet_with_visible_amounts"("_timesheet_id" "uuid") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."get_tour_complete_timeline"("p_tour_id" "uuid") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."get_tour_date_complete_info"("p_tour_date_id" "uuid") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."get_user_job_ids"("user_uuid" "uuid") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."get_waha_config"("base_url" "text") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."handle_new_user"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."handle_updated_at"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."increment_timesheet_version"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."invoke_scheduled_push_notification"("event_type" "text") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."is_admin"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."is_house_tech"("_profile_id" "uuid") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."is_management_or_admin"("p_user_id" "uuid") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."iso_year_week_madrid"("ts" timestamp with time zone) TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."json_diff_public"("_old" "jsonb", "_new" "jsonb", "allowed" "text"[]) TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."log_activity"("_code" "text", "_job_id" "uuid", "_entity_type" "text", "_entity_id" "text", "_payload" "jsonb", "_visibility" "public"."activity_visibility") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."log_activity_as"("_actor_id" "uuid", "_code" "text", "_job_id" "uuid", "_entity_type" "text", "_entity_id" "text", "_payload" "jsonb", "_visibility" "public"."activity_visibility") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."log_timesheet_changes"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."minutes_to_hours_round_30"("mins" integer) TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."needs_vehicle_disclaimer"("_profile_id" "uuid") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."normalize_text_for_match"("input" "text") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."notify_direct_message"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."presets_set_department_from_user"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."refresh_soundvision_file_review_stats"() TO "dreamlit_app";



GRANT ALL ON FUNCTION "public"."remove_assignment_with_timesheets"("p_job_id" "uuid", "p_technician_id" "uuid") TO "anon";
...
GRANT ALL ON FUNCTION "public"."remove_assignment_with_timesheets"("p_job_id" "uuid", "p_technician_id" "uuid") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."resolve_category_for_timesheet"("_job_id" "uuid", "_tech_id" "uuid") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."resolve_visibility"("_code" "text", "_job_id" "uuid", "_actor_id" "uuid") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."rotate_my_calendar_ics_token"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."set_updated_at"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."sub_rentals_set_department_from_equipment"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."sync_existing_tour_assignments_to_new_job"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."sync_preset_assignments_for_tour"("_preset_id" "uuid", "_tour_id" "uuid") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."sync_tour_assignments_to_jobs"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."sync_tour_start_end_dates"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."sync_vacations_to_availability"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."tg_touch_updated_at"() TO "dreamlit_app";



GRANT ALL ON FUNCTION "public"."toggle_timesheet_day"("p_job_id" "uuid", "p_technician_id" "uuid", "p_date" "date", "p_present" boolean, "p_source" "text") TO "anon";
...
GRANT ALL ON FUNCTION "public"."toggle_timesheet_day"("p_job_id" "uuid", "p_technician_id" "uuid", "p_date" "date", "p_present" boolean, "p_source" "text") TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."touch_activity_prefs_updated_at"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."touch_soundvision_file_reviews_updated_at"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_job_required_roles_set_updated_at"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_jobs_sync_tour_preset_assignments"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_log_assignment_delete"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_log_assignment_insert"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_log_assignment_update"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_log_document_delete"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_log_document_insert"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_log_hoja_update"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_log_job_created"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_log_job_updated"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_log_staffing_update"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_log_timesheet_insert"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_log_timesheet_update"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_log_tourdate_delete"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_log_tourdate_insert"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_log_tourdate_update"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_presets_sync_tour_assignments"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."trg_timesheets_autofill_category"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."update_hoja_de_ruta_last_modified"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."update_job_flex_folders_flag"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."update_morning_subscription_updated_at"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."update_push_schedule_updated_at"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."update_task_status_on_document_delete"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."update_tour_dates"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."update_updated_at_column"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."update_vacation_requests_updated_at"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."update_venues_updated_at"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."upsert_venue"("p_name" "text", "p_google_place_id" "text", "p_city" "text", "p_state_region" "text", "p_country" "text", "p_full_address" "text", "p_coordinates" "jsonb", "p_capacity" integer) TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."validate_timesheet_status_transition"() TO "dreamlit_app";
...
GRANT ALL ON FUNCTION "public"."validate_tour_date_job"() TO "dreamlit_app";



REVOKE ALL ON FUNCTION "secrets"."get_waha_config"("base_url" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "secrets"."get_waha_config"("base_url" "text") TO "dreamlit_app";
GRANT ALL ON FUNCTION "secrets"."get_waha_config"("base_url" "text") TO "service_role";



GRANT ALL ON FUNCTION "secrets"."set_updated_at"() TO "dreamlit_app";
...
GRANT ALL ON TABLE "dreamlit"."error_log" TO "dreamlit_app";
...
GRANT SELECT,USAGE ON SEQUENCE "dreamlit"."error_log_id_seq" TO "dreamlit_app";
...
GRANT ALL ON TABLE "dreamlit"."event_log" TO "dreamlit_app";
...
GRANT ALL ON TABLE "dreamlit"."version" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."activity_catalog" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."activity_log" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."activity_prefs" TO "anon";
GRANT ALL ON TABLE "public"."activity_prefs" TO "authenticated";
GRANT ALL ON TABLE "public"."activity_prefs" TO "service_role";
GRANT ALL ON TABLE "public"."activity_prefs" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."activity_reads" TO "anon";
GRANT ALL ON TABLE "public"."activity_reads" TO "authenticated";
GRANT ALL ON TABLE "public"."activity_reads" TO "service_role";
GRANT ALL ON TABLE "public"."activity_reads" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."announcements" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."app_changelog" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."assignment_notifications" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."availability_conflicts" TO "anon";
GRANT ALL ON TABLE "public"."availability_conflicts" TO "authenticated";
GRANT ALL ON TABLE "public"."availability_conflicts" TO "service_role";
GRANT ALL ON TABLE "public"."availability_conflicts" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."availability_exceptions" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."availability_schedules" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."corporate_email_logs" TO "anon";
GRANT ALL ON TABLE "public"."corporate_email_logs" TO "authenticated";
GRANT ALL ON TABLE "public"."corporate_email_logs" TO "service_role";
GRANT ALL ON TABLE "public"."corporate_email_logs" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."equipment" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."global_stock_entries" TO "anon";
GRANT ALL ON TABLE "public"."global_stock_entries" TO "authenticated";
GRANT ALL ON TABLE "public"."global_stock_entries" TO "service_role";
GRANT ALL ON TABLE "public"."global_stock_entries" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."current_stock_levels" TO "anon";
GRANT ALL ON TABLE "public"."current_stock_levels" TO "authenticated";
GRANT ALL ON TABLE "public"."current_stock_levels" TO "service_role";
GRANT ALL ON TABLE "public"."current_stock_levels" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."day_assignments" TO "anon";
GRANT ALL ON TABLE "public"."day_assignments" TO "authenticated";
GRANT ALL ON TABLE "public"."day_assignments" TO "service_role";
GRANT ALL ON TABLE "public"."day_assignments" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."day_preset_assignments" TO "anon";
GRANT ALL ON TABLE "public"."day_preset_assignments" TO "authenticated";
GRANT ALL ON TABLE "public"."day_preset_assignments" TO "service_role";
GRANT ALL ON TABLE "public"."day_preset_assignments" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."direct_messages" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."dwg_conversion_queue" TO "anon";
GRANT ALL ON TABLE "public"."dwg_conversion_queue" TO "authenticated";
GRANT ALL ON TABLE "public"."dwg_conversion_queue" TO "service_role";
GRANT ALL ON TABLE "public"."dwg_conversion_queue" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."sub_rentals" TO "anon";
GRANT ALL ON TABLE "public"."sub_rentals" TO "authenticated";
GRANT ALL ON TABLE "public"."sub_rentals" TO "service_role";
GRANT ALL ON TABLE "public"."sub_rentals" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."equipment_availability_with_rentals" TO "anon";
GRANT ALL ON TABLE "public"."equipment_availability_with_rentals" TO "authenticated";
GRANT ALL ON TABLE "public"."equipment_availability_with_rentals" TO "service_role";
GRANT ALL ON TABLE "public"."equipment_availability_with_rentals" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."equipment_models" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."festival_artist_files" TO "anon";
GRANT ALL ON TABLE "public"."festival_artist_files" TO "authenticated";
GRANT ALL ON TABLE "public"."festival_artist_files" TO "service_role";
GRANT ALL ON TABLE "public"."festival_artist_files" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."festival_artist_form_submissions" TO "anon";
GRANT ALL ON TABLE "public"."festival_artist_form_submissions" TO "authenticated";
GRANT ALL ON TABLE "public"."festival_artist_form_submissions" TO "service_role";
GRANT ALL ON TABLE "public"."festival_artist_form_submissions" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."festival_artist_forms" TO "anon";
GRANT ALL ON TABLE "public"."festival_artist_forms" TO "authenticated";
GRANT ALL ON TABLE "public"."festival_artist_forms" TO "service_role";
GRANT ALL ON TABLE "public"."festival_artist_forms" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."festival_artists" TO "anon";
GRANT ALL ON TABLE "public"."festival_artists" TO "authenticated";
GRANT ALL ON TABLE "public"."festival_artists" TO "service_role";
GRANT ALL ON TABLE "public"."festival_artists" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."festival_gear_setups" TO "anon";
GRANT ALL ON TABLE "public"."festival_gear_setups" TO "authenticated";
GRANT ALL ON TABLE "public"."festival_gear_setups" TO "service_role";
GRANT ALL ON TABLE "public"."festival_gear_setups" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."festival_logos" TO "anon";
GRANT ALL ON TABLE "public"."festival_logos" TO "authenticated";
GRANT ALL ON TABLE "public"."festival_logos" TO "service_role";
GRANT ALL ON TABLE "public"."festival_logos" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."festival_settings" TO "anon";
GRANT ALL ON TABLE "public"."festival_settings" TO "authenticated";
GRANT ALL ON TABLE "public"."festival_settings" TO "service_role";
GRANT ALL ON TABLE "public"."festival_settings" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."festival_shift_assignments" TO "anon";
GRANT ALL ON TABLE "public"."festival_shift_assignments" TO "authenticated";
GRANT ALL ON TABLE "public"."festival_shift_assignments" TO "service_role";
GRANT ALL ON TABLE "public"."festival_shift_assignments" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."festival_shifts" TO "anon";
GRANT ALL ON TABLE "public"."festival_shifts" TO "authenticated";
GRANT ALL ON TABLE "public"."festival_shifts" TO "service_role";
GRANT ALL ON TABLE "public"."festival_shifts" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."festival_stage_gear_setups" TO "anon";
GRANT ALL ON TABLE "public"."festival_stage_gear_setups" TO "authenticated";
GRANT ALL ON TABLE "public"."festival_stage_gear_setups" TO "service_role";
GRANT ALL ON TABLE "public"."festival_stage_gear_setups" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."festival_stages" TO "anon";
GRANT ALL ON TABLE "public"."festival_stages" TO "authenticated";
GRANT ALL ON TABLE "public"."festival_stages" TO "service_role";
GRANT ALL ON TABLE "public"."festival_stages" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."flex_crew_assignments" TO "anon";
GRANT ALL ON TABLE "public"."flex_crew_assignments" TO "authenticated";
GRANT ALL ON TABLE "public"."flex_crew_assignments" TO "service_role";
GRANT ALL ON TABLE "public"."flex_crew_assignments" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."flex_crew_calls" TO "anon";
GRANT ALL ON TABLE "public"."flex_crew_calls" TO "authenticated";
GRANT ALL ON TABLE "public"."flex_crew_calls" TO "service_role";
GRANT ALL ON TABLE "public"."flex_crew_calls" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."flex_folders" TO "anon";
GRANT ALL ON TABLE "public"."flex_folders" TO "authenticated";
GRANT ALL ON TABLE "public"."flex_folders" TO "service_role";
GRANT ALL ON TABLE "public"."flex_folders" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."flex_status_log" TO "anon";
GRANT ALL ON TABLE "public"."flex_status_log" TO "authenticated";
GRANT ALL ON TABLE "public"."flex_status_log" TO "service_role";
GRANT ALL ON TABLE "public"."flex_status_log" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."flex_work_order_items" TO "anon";
GRANT ALL ON TABLE "public"."flex_work_order_items" TO "authenticated";
GRANT ALL ON TABLE "public"."flex_work_order_items" TO "service_role";
GRANT ALL ON TABLE "public"."flex_work_order_items" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."flex_work_orders" TO "anon";
GRANT ALL ON TABLE "public"."flex_work_orders" TO "authenticated";
GRANT ALL ON TABLE "public"."flex_work_orders" TO "service_role";
GRANT ALL ON TABLE "public"."flex_work_orders" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."global_availability_presets" TO "anon";
GRANT ALL ON TABLE "public"."global_availability_presets" TO "authenticated";
GRANT ALL ON TABLE "public"."global_availability_presets" TO "service_role";
GRANT ALL ON TABLE "public"."global_availability_presets" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."hoja_de_ruta" TO "anon";
GRANT ALL ON TABLE "public"."hoja_de_ruta" TO "authenticated";
GRANT ALL ON TABLE "public"."hoja_de_ruta" TO "service_role";
GRANT ALL ON TABLE "public"."hoja_de_ruta" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."hoja_de_ruta_accommodations" TO "anon";
GRANT ALL ON TABLE "public"."hoja_de_ruta_accommodations" TO "authenticated";
GRANT ALL ON TABLE "public"."hoja_de_ruta_accommodations" TO "service_role";
GRANT ALL ON TABLE "public"."hoja_de_ruta_accommodations" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."hoja_de_ruta_contacts" TO "anon";
GRANT ALL ON TABLE "public"."hoja_de_ruta_contacts" TO "authenticated";
GRANT ALL ON TABLE "public"."hoja_de_ruta_contacts" TO "service_role";
GRANT ALL ON TABLE "public"."hoja_de_ruta_contacts" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."hoja_de_ruta_equipment" TO "anon";
GRANT ALL ON TABLE "public"."hoja_de_ruta_equipment" TO "authenticated";
GRANT ALL ON TABLE "public"."hoja_de_ruta_equipment" TO "service_role";
GRANT ALL ON TABLE "public"."hoja_de_ruta_equipment" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."hoja_de_ruta_images" TO "anon";
GRANT ALL ON TABLE "public"."hoja_de_ruta_images" TO "authenticated";
GRANT ALL ON TABLE "public"."hoja_de_ruta_images" TO "service_role";
GRANT ALL ON TABLE "public"."hoja_de_ruta_images" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."hoja_de_ruta_logistics" TO "anon";
GRANT ALL ON TABLE "public"."hoja_de_ruta_logistics" TO "authenticated";
GRANT ALL ON TABLE "public"."hoja_de_ruta_logistics" TO "service_role";
GRANT ALL ON TABLE "public"."hoja_de_ruta_logistics" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."hoja_de_ruta_restaurants" TO "anon";
GRANT ALL ON TABLE "public"."hoja_de_ruta_restaurants" TO "authenticated";
GRANT ALL ON TABLE "public"."hoja_de_ruta_restaurants" TO "service_role";
GRANT ALL ON TABLE "public"."hoja_de_ruta_restaurants" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."hoja_de_ruta_room_assignments" TO "anon";
GRANT ALL ON TABLE "public"."hoja_de_ruta_room_assignments" TO "authenticated";
GRANT ALL ON TABLE "public"."hoja_de_ruta_room_assignments" TO "service_role";
GRANT ALL ON TABLE "public"."hoja_de_ruta_room_assignments" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."hoja_de_ruta_rooms" TO "anon";
GRANT ALL ON TABLE "public"."hoja_de_ruta_rooms" TO "authenticated";
GRANT ALL ON TABLE "public"."hoja_de_ruta_rooms" TO "service_role";
GRANT ALL ON TABLE "public"."hoja_de_ruta_rooms" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."hoja_de_ruta_staff" TO "anon";
GRANT ALL ON TABLE "public"."hoja_de_ruta_staff" TO "authenticated";
GRANT ALL ON TABLE "public"."hoja_de_ruta_staff" TO "service_role";
GRANT ALL ON TABLE "public"."hoja_de_ruta_staff" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."hoja_de_ruta_templates" TO "anon";
GRANT ALL ON TABLE "public"."hoja_de_ruta_templates" TO "authenticated";
GRANT ALL ON TABLE "public"."hoja_de_ruta_templates" TO "service_role";
GRANT ALL ON TABLE "public"."hoja_de_ruta_templates" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."hoja_de_ruta_transport" TO "anon";
GRANT ALL ON TABLE "public"."hoja_de_ruta_transport" TO "authenticated";
GRANT ALL ON TABLE "public"."hoja_de_ruta_transport" TO "service_role";
GRANT ALL ON TABLE "public"."hoja_de_ruta_transport" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."hoja_de_ruta_travel" TO "anon";
GRANT ALL ON TABLE "public"."hoja_de_ruta_travel" TO "authenticated";
GRANT ALL ON TABLE "public"."hoja_de_ruta_travel" TO "service_role";
GRANT ALL ON TABLE "public"."hoja_de_ruta_travel" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."hoja_de_ruta_travel_arrangements" TO "anon";
GRANT ALL ON TABLE "public"."hoja_de_ruta_travel_arrangements" TO "authenticated";
GRANT ALL ON TABLE "public"."hoja_de_ruta_travel_arrangements" TO "service_role";
GRANT ALL ON TABLE "public"."hoja_de_ruta_travel_arrangements" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."house_tech_rates" TO "anon";
GRANT ALL ON TABLE "public"."house_tech_rates" TO "authenticated";
GRANT ALL ON TABLE "public"."house_tech_rates" TO "service_role";
GRANT ALL ON TABLE "public"."house_tech_rates" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."job_assignments" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."job_date_types" TO "anon";
GRANT ALL ON TABLE "public"."job_date_types" TO "authenticated";
GRANT ALL ON TABLE "public"."job_date_types" TO "service_role";
GRANT ALL ON TABLE "public"."job_date_types" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."job_departments" TO "anon";
GRANT ALL ON TABLE "public"."job_departments" TO "authenticated";
GRANT ALL ON TABLE "public"."job_departments" TO "service_role";
GRANT ALL ON TABLE "public"."job_departments" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."job_documents" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."job_milestone_definitions" TO "anon";
GRANT ALL ON TABLE "public"."job_milestone_definitions" TO "authenticated";
GRANT ALL ON TABLE "public"."job_milestone_definitions" TO "service_role";
GRANT ALL ON TABLE "public"."job_milestone_definitions" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."job_milestones" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."job_rate_extras" TO "anon";
GRANT ALL ON TABLE "public"."job_rate_extras" TO "authenticated";
GRANT ALL ON TABLE "public"."job_rate_extras" TO "service_role";
GRANT ALL ON TABLE "public"."job_rate_extras" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."job_required_roles" TO "anon";
GRANT ALL ON TABLE "public"."job_required_roles" TO "authenticated";
GRANT ALL ON TABLE "public"."job_required_roles" TO "service_role";
GRANT ALL ON TABLE "public"."job_required_roles" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."job_required_roles_summary" TO "anon";
GRANT ALL ON TABLE "public"."job_required_roles_summary" TO "authenticated";
GRANT ALL ON TABLE "public"."job_required_roles_summary" TO "service_role";
GRANT ALL ON TABLE "public"."job_required_roles_summary" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."job_whatsapp_group_requests" TO "anon";
GRANT ALL ON TABLE "public"."job_whatsapp_group_requests" TO "authenticated";
GRANT ALL ON TABLE "public"."job_whatsapp_group_requests" TO "service_role";
GRANT ALL ON TABLE "public"."job_whatsapp_group_requests" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."job_whatsapp_groups" TO "anon";
GRANT ALL ON TABLE "public"."job_whatsapp_groups" TO "authenticated";
GRANT ALL ON TABLE "public"."job_whatsapp_groups" TO "service_role";
GRANT ALL ON TABLE "public"."job_whatsapp_groups" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."jobs" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."lights_job_personnel" TO "anon";
GRANT ALL ON TABLE "public"."lights_job_personnel" TO "authenticated";
GRANT ALL ON TABLE "public"."lights_job_personnel" TO "service_role";
GRANT ALL ON TABLE "public"."lights_job_personnel" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."lights_job_tasks" TO "anon";
GRANT ALL ON TABLE "public"."lights_job_tasks" TO "authenticated";
GRANT ALL ON TABLE "public"."lights_job_tasks" TO "service_role";
GRANT ALL ON TABLE "public"."lights_job_tasks" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."lights_memoria_tecnica_documents" TO "anon";
GRANT ALL ON TABLE "public"."lights_memoria_tecnica_documents" TO "authenticated";
GRANT ALL ON TABLE "public"."lights_memoria_tecnica_documents" TO "service_role";
GRANT ALL ON TABLE "public"."lights_memoria_tecnica_documents" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."locations" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."logistics_event_departments" TO "anon";
GRANT ALL ON TABLE "public"."logistics_event_departments" TO "authenticated";
GRANT ALL ON TABLE "public"."logistics_event_departments" TO "service_role";
GRANT ALL ON TABLE "public"."logistics_event_departments" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."logistics_events" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."memoria_tecnica_documents" TO "anon";
GRANT ALL ON TABLE "public"."memoria_tecnica_documents" TO "authenticated";
GRANT ALL ON TABLE "public"."memoria_tecnica_documents" TO "service_role";
GRANT ALL ON TABLE "public"."memoria_tecnica_documents" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."messages" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."milestone_definitions" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."morning_summary_subscriptions" TO "anon";
GRANT ALL ON TABLE "public"."morning_summary_subscriptions" TO "authenticated";
GRANT ALL ON TABLE "public"."morning_summary_subscriptions" TO "service_role";
GRANT ALL ON TABLE "public"."morning_summary_subscriptions" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."notification_preferences" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."notification_subscriptions" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."profiles" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."sound_job_tasks" TO "anon";
GRANT ALL ON TABLE "public"."sound_job_tasks" TO "authenticated";
GRANT ALL ON TABLE "public"."sound_job_tasks" TO "service_role";
GRANT ALL ON TABLE "public"."sound_job_tasks" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."video_job_tasks" TO "anon";
GRANT ALL ON TABLE "public"."video_job_tasks" TO "authenticated";
GRANT ALL ON TABLE "public"."video_job_tasks" TO "service_role";
GRANT ALL ON TABLE "public"."video_job_tasks" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."pending_tasks_view" TO "anon";
GRANT ALL ON TABLE "public"."pending_tasks_view" TO "authenticated";
GRANT ALL ON TABLE "public"."pending_tasks_view" TO "service_role";
GRANT ALL ON TABLE "public"."pending_tasks_view" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."power_requirement_tables" TO "anon";
GRANT ALL ON TABLE "public"."power_requirement_tables" TO "authenticated";
GRANT ALL ON TABLE "public"."power_requirement_tables" TO "service_role";
GRANT ALL ON TABLE "public"."power_requirement_tables" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."preset_items" TO "anon";
GRANT ALL ON TABLE "public"."preset_items" TO "authenticated";
GRANT ALL ON TABLE "public"."preset_items" TO "service_role";
GRANT ALL ON TABLE "public"."preset_items" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."presets" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."profile_skills" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."push_cron_config" TO "anon";
GRANT ALL ON TABLE "public"."push_cron_config" TO "authenticated";
GRANT ALL ON TABLE "public"."push_cron_config" TO "service_role";
GRANT ALL ON TABLE "public"."push_cron_config" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."push_cron_execution_log" TO "anon";
GRANT ALL ON TABLE "public"."push_cron_execution_log" TO "authenticated";
GRANT ALL ON TABLE "public"."push_cron_execution_log" TO "service_role";
GRANT ALL ON TABLE "public"."push_cron_execution_log" TO "dreamlit_app";
...
GRANT SELECT,USAGE ON SEQUENCE "public"."push_cron_execution_log_id_seq" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."push_notification_routes" TO "anon";
GRANT ALL ON TABLE "public"."push_notification_routes" TO "authenticated";
GRANT ALL ON TABLE "public"."push_notification_routes" TO "service_role";
GRANT ALL ON TABLE "public"."push_notification_routes" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."push_notification_schedules" TO "anon";
GRANT ALL ON TABLE "public"."push_notification_schedules" TO "authenticated";
GRANT ALL ON TABLE "public"."push_notification_schedules" TO "service_role";
GRANT ALL ON TABLE "public"."push_notification_schedules" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."push_subscriptions" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."rate_cards_2025" TO "anon";
GRANT ALL ON TABLE "public"."rate_cards_2025" TO "authenticated";
GRANT ALL ON TABLE "public"."rate_cards_2025" TO "service_role";
GRANT ALL ON TABLE "public"."rate_cards_2025" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."rate_cards_tour_2025" TO "anon";
GRANT ALL ON TABLE "public"."rate_cards_tour_2025" TO "authenticated";
GRANT ALL ON TABLE "public"."rate_cards_tour_2025" TO "service_role";
GRANT ALL ON TABLE "public"."rate_cards_tour_2025" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."rate_extras_2025" TO "anon";
GRANT ALL ON TABLE "public"."rate_extras_2025" TO "authenticated";
GRANT ALL ON TABLE "public"."rate_extras_2025" TO "service_role";
GRANT ALL ON TABLE "public"."rate_extras_2025" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."required_docs" TO "anon";
GRANT ALL ON TABLE "public"."required_docs" TO "authenticated";
GRANT ALL ON TABLE "public"."required_docs" TO "service_role";
GRANT ALL ON TABLE "public"."required_docs" TO "dreamlit_app";
...
GRANT SELECT,USAGE ON SEQUENCE "public"."required_docs_id_seq" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."secrets" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."skills" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."sound_job_personnel" TO "anon";
GRANT ALL ON TABLE "public"."sound_job_personnel" TO "authenticated";
GRANT ALL ON TABLE "public"."sound_job_personnel" TO "service_role";
GRANT ALL ON TABLE "public"."sound_job_personnel" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."soundvision_file_reviews" TO "anon";
GRANT ALL ON TABLE "public"."soundvision_file_reviews" TO "authenticated";
GRANT ALL ON TABLE "public"."soundvision_file_reviews" TO "service_role";
GRANT ALL ON TABLE "public"."soundvision_file_reviews" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."soundvision_files" TO "anon";
GRANT ALL ON TABLE "public"."soundvision_files" TO "authenticated";
GRANT ALL ON TABLE "public"."soundvision_files" TO "service_role";
GRANT ALL ON TABLE "public"."soundvision_files" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."staffing_events" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."staffing_requests" TO "anon";
GRANT ALL ON TABLE "public"."staffing_requests" TO "authenticated";
GRANT ALL ON TABLE "public"."staffing_requests" TO "service_role";
GRANT ALL ON TABLE "public"."staffing_requests" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."stock_movements" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."system_errors" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."system_health_assignments" TO "anon";
GRANT ALL ON TABLE "public"."system_health_assignments" TO "authenticated";
GRANT ALL ON TABLE "public"."system_health_assignments" TO "service_role";
GRANT ALL ON TABLE "public"."system_health_assignments" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."timesheets" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."system_health_timesheets" TO "anon";
GRANT ALL ON TABLE "public"."system_health_timesheets" TO "authenticated";
GRANT ALL ON TABLE "public"."system_health_timesheets" TO "service_role";
GRANT ALL ON TABLE "public"."system_health_timesheets" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."task_documents" TO "anon";
GRANT ALL ON TABLE "public"."task_documents" TO "authenticated";
GRANT ALL ON TABLE "public"."task_documents" TO "service_role";
GRANT ALL ON TABLE "public"."task_documents" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."technician_availability" TO "dreamlit_app";
...
GRANT SELECT,USAGE ON SEQUENCE "public"."technician_availability_id_seq" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."technician_departments" TO "anon";
GRANT ALL ON TABLE "public"."technician_departments" TO "authenticated";
GRANT ALL ON TABLE "public"."technician_departments" TO "service_role";
GRANT ALL ON TABLE "public"."technician_departments" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."technician_fridge" TO "anon";
GRANT ALL ON TABLE "public"."technician_fridge" TO "authenticated";
GRANT ALL ON TABLE "public"."technician_fridge" TO "service_role";
GRANT ALL ON TABLE "public"."technician_fridge" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."technician_work_records" TO "anon";
GRANT ALL ON TABLE "public"."technician_work_records" TO "authenticated";
GRANT ALL ON TABLE "public"."technician_work_records" TO "service_role";
GRANT ALL ON TABLE "public"."technician_work_records" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."timesheet_audit_log" TO "anon";
GRANT ALL ON TABLE "public"."timesheet_audit_log" TO "authenticated";
GRANT ALL ON TABLE "public"."timesheet_audit_log" TO "service_role";
GRANT ALL ON TABLE "public"."timesheet_audit_log" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."tour_accommodations" TO "anon";
GRANT ALL ON TABLE "public"."tour_accommodations" TO "authenticated";
GRANT ALL ON TABLE "public"."tour_accommodations" TO "service_role";
GRANT ALL ON TABLE "public"."tour_accommodations" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."tour_assignments" TO "anon";
GRANT ALL ON TABLE "public"."tour_assignments" TO "authenticated";
GRANT ALL ON TABLE "public"."tour_assignments" TO "service_role";
GRANT ALL ON TABLE "public"."tour_assignments" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."tour_date_power_overrides" TO "anon";
GRANT ALL ON TABLE "public"."tour_date_power_overrides" TO "authenticated";
GRANT ALL ON TABLE "public"."tour_date_power_overrides" TO "service_role";
GRANT ALL ON TABLE "public"."tour_date_power_overrides" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."tour_date_weight_overrides" TO "anon";
GRANT ALL ON TABLE "public"."tour_date_weight_overrides" TO "authenticated";
GRANT ALL ON TABLE "public"."tour_date_weight_overrides" TO "service_role";
GRANT ALL ON TABLE "public"."tour_date_weight_overrides" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."tour_dates" TO "anon";
GRANT ALL ON TABLE "public"."tour_dates" TO "authenticated";
GRANT ALL ON TABLE "public"."tour_dates" TO "service_role";
GRANT ALL ON TABLE "public"."tour_dates" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."tour_default_sets" TO "anon";
GRANT ALL ON TABLE "public"."tour_default_sets" TO "authenticated";
GRANT ALL ON TABLE "public"."tour_default_sets" TO "service_role";
GRANT ALL ON TABLE "public"."tour_default_sets" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."tour_default_tables" TO "anon";
GRANT ALL ON TABLE "public"."tour_default_tables" TO "authenticated";
GRANT ALL ON TABLE "public"."tour_default_tables" TO "service_role";
GRANT ALL ON TABLE "public"."tour_default_tables" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."tour_documents" TO "anon";
GRANT ALL ON TABLE "public"."tour_documents" TO "authenticated";
GRANT ALL ON TABLE "public"."tour_documents" TO "service_role";
GRANT ALL ON TABLE "public"."tour_documents" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."tour_logos" TO "anon";
GRANT ALL ON TABLE "public"."tour_logos" TO "authenticated";
GRANT ALL ON TABLE "public"."tour_logos" TO "service_role";
GRANT ALL ON TABLE "public"."tour_logos" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."tour_power_defaults" TO "anon";
GRANT ALL ON TABLE "public"."tour_power_defaults" TO "authenticated";
GRANT ALL ON TABLE "public"."tour_power_defaults" TO "service_role";
GRANT ALL ON TABLE "public"."tour_power_defaults" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."tour_schedule_templates" TO "anon";
GRANT ALL ON TABLE "public"."tour_schedule_templates" TO "authenticated";
GRANT ALL ON TABLE "public"."tour_schedule_templates" TO "service_role";
GRANT ALL ON TABLE "public"."tour_schedule_templates" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."tour_timeline_events" TO "anon";
GRANT ALL ON TABLE "public"."tour_timeline_events" TO "authenticated";
GRANT ALL ON TABLE "public"."tour_timeline_events" TO "service_role";
GRANT ALL ON TABLE "public"."tour_timeline_events" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."tour_travel_segments" TO "anon";
GRANT ALL ON TABLE "public"."tour_travel_segments" TO "authenticated";
GRANT ALL ON TABLE "public"."tour_travel_segments" TO "service_role";
GRANT ALL ON TABLE "public"."tour_travel_segments" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."tour_week_multipliers_2025" TO "anon";
GRANT ALL ON TABLE "public"."tour_week_multipliers_2025" TO "authenticated";
GRANT ALL ON TABLE "public"."tour_week_multipliers_2025" TO "service_role";
GRANT ALL ON TABLE "public"."tour_week_multipliers_2025" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."tour_weight_defaults" TO "anon";
GRANT ALL ON TABLE "public"."tour_weight_defaults" TO "authenticated";
GRANT ALL ON TABLE "public"."tour_weight_defaults" TO "service_role";
GRANT ALL ON TABLE "public"."tour_weight_defaults" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."tours" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."transport_request_items" TO "anon";
GRANT ALL ON TABLE "public"."transport_request_items" TO "authenticated";
GRANT ALL ON TABLE "public"."transport_request_items" TO "service_role";
GRANT ALL ON TABLE "public"."transport_request_items" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."transport_requests" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."v_job_tech_payout_2025" TO "anon";
GRANT ALL ON TABLE "public"."v_job_tech_payout_2025" TO "authenticated";
GRANT ALL ON TABLE "public"."v_job_tech_payout_2025" TO "service_role";
GRANT ALL ON TABLE "public"."v_job_tech_payout_2025" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."v_tour_job_rate_quotes_2025" TO "anon";
GRANT ALL ON TABLE "public"."v_tour_job_rate_quotes_2025" TO "authenticated";
GRANT ALL ON TABLE "public"."v_tour_job_rate_quotes_2025" TO "service_role";
GRANT ALL ON TABLE "public"."v_tour_job_rate_quotes_2025" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."vacation_requests" TO "dreamlit_app";
...
GRANT ALL ON TABLE "public"."venues" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."video_job_personnel" TO "anon";
GRANT ALL ON TABLE "public"."video_job_personnel" TO "authenticated";
GRANT ALL ON TABLE "public"."video_job_personnel" TO "service_role";
GRANT ALL ON TABLE "public"."video_job_personnel" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."video_memoria_tecnica_documents" TO "anon";
GRANT ALL ON TABLE "public"."video_memoria_tecnica_documents" TO "authenticated";
GRANT ALL ON TABLE "public"."video_memoria_tecnica_documents" TO "service_role";
GRANT ALL ON TABLE "public"."video_memoria_tecnica_documents" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."wallboard_doc_counts" TO "anon";
GRANT ALL ON TABLE "public"."wallboard_doc_counts" TO "authenticated";
GRANT ALL ON TABLE "public"."wallboard_doc_counts" TO "service_role";
GRANT ALL ON TABLE "public"."wallboard_doc_counts" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."wallboard_doc_requirements" TO "anon";
GRANT ALL ON TABLE "public"."wallboard_doc_requirements" TO "authenticated";
GRANT ALL ON TABLE "public"."wallboard_doc_requirements" TO "service_role";
GRANT ALL ON TABLE "public"."wallboard_doc_requirements" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."wallboard_presets" TO "anon";
GRANT ALL ON TABLE "public"."wallboard_presets" TO "authenticated";
GRANT ALL ON TABLE "public"."wallboard_presets" TO "service_role";
GRANT ALL ON TABLE "public"."wallboard_presets" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."wallboard_profiles" TO "anon";
GRANT ALL ON TABLE "public"."wallboard_profiles" TO "authenticated";
GRANT ALL ON TABLE "public"."wallboard_profiles" TO "service_role";
GRANT ALL ON TABLE "public"."wallboard_profiles" TO "dreamlit_app";



GRANT ALL ON TABLE "public"."wallboard_timesheet_status" TO "anon";
GRANT ALL ON TABLE "public"."wallboard_timesheet_status" TO "authenticated";
GRANT ALL ON TABLE "public"."wallboard_timesheet_status" TO "service_role";
GRANT ALL ON TABLE "public"."wallboard_timesheet_status" TO "dreamlit_app";



GRANT ALL ON TABLE "secrets"."waha_hosts" TO "dreamlit_app";
GRANT SELECT,INSERT,DELETE,UPDATE ON TABLE "secrets"."waha_hosts" TO "service_role";



ALTER DEFAULT PRIVILEGES FOR ROLE "dreamlit_app" IN SCHEMA "dreamlit" GRANT SELECT,USAGE ON SEQUENCES  TO "postgres";



ALTER DEFAULT PRIVILEGES FOR ROLE "dreamlit_app" IN SCHEMA "dreamlit" GRANT ALL ON FUNCTIONS  TO "postgres";



ALTER DEFAULT PRIVILEGES FOR ROLE "dreamlit_app" IN SCHEMA "dreamlit" GRANT ALL ON TABLES  TO "postgres";
...
ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" GRANT SELECT,USAGE ON SEQUENCES  TO "dreamlit_app";



ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" GRANT ALL ON FUNCTIONS  TO "dreamlit_app";



ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" GRANT ALL ON SCHEMAS  TO "dreamlit_app";



ALTER DEFAULT PRIVILEGES FOR ROLE "postgres" GRANT ALL ON TABLES  TO "dreamlit_app";
