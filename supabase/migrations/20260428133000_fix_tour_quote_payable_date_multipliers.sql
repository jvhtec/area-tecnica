-- Fix tour quote payable-date handling after prep-day support:
-- - scheduled non-prep job_date_types remain the base payable dates even when
--   only some timesheets have been submitted.
-- - single-day technician assignments still scope a tech to those assignment dates.
-- - weekly multipliers are applied per standard payable date, so expanded
--   rigging/show dates and cross-week tour dates price correctly.
...
BEGIN
  ...
  WITH scheduled_job_date_type_dates AS (
    SELECT DISTINCT jdt.date AS payable_date
    FROM public.job_date_types jdt
    WHERE jdt.job_id = _job_id
      AND jdt.type <> 'prep_day'
  ),
  ...
  fallback_schedule_dates AS (
    ...
    WHERE NOT EXISTS (SELECT 1 FROM scheduled_job_date_type_dates)
      AND NOT EXISTS (
        SELECT 1
        FROM public.job_date_types prep_jdt
  ...
  scheduled_days := GREATEST(COALESCE(scheduled_days, 0), 1);
  ...
  IF standard_days > 0 THEN
    ...
    -- scheduled_job_date_type_dates, active_timesheet_dates,
    -- single_day_assignment_dates, and fallback_schedule_dates in sync with
    -- the first copy, or refactor them into a shared SQL function.
    WITH scheduled_job_date_type_dates AS (
      SELECT DISTINCT jdt.date AS payable_date
      FROM public.job_date_types jdt
      WHERE jdt.job_id = _job_id
    ...
    date_multipliers AS (
      ...
      LEFT JOIN LATERAL (
        SELECT COUNT(DISTINCT pd.date)::int AS cnt
        FROM public.jobs j
        ...
        CROSS JOIN LATERAL public.iso_year_week_madrid(pd.date::timestamptz) other_iw
        WHERE j.job_type = 'tourdate'
          AND j.tour_id = tour_group
          AND j.status != 'Cancelado'
          AND other_iw.iso_year = iw.iso_year
          AND other_iw.iso_week = iw.iso_week
          AND NOT EXISTS (
            SELECT 1
            FROM public.job_date_types only_prep
            WHERE only_prep.job_id = j.id
              AND only_prep.type = 'prep_day'
              AND NOT EXISTS (
                SELECT 1
                FROM public.job_date_types non_prep
                WHERE non_prep.job_id = j.id
                  AND non_prep.type <> 'prep_day'
              )
          )
...
COMMENT ON FUNCTION public.compute_tour_job_rate_quote_2025(uuid,uuid) IS
  'Calculates tour job rate quotes. Scheduled non-prep job_date_types are the base payable dates, so expanded rigging/show days update quotes even before all timesheets exist. Weekly multipliers are applied per technician per eligible payable date/week; prep days are fixed-rate timesheets and excluded from quote counts.';
