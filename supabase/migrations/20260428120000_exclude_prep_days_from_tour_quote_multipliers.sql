-- Prep days are fixed-rate timesheet days and must not expand the tour quote
-- scheduled span or weekly tour multiplier counts.
...
BEGIN
  ...
  -- job_date_types is the canonical scheduled span for payout logic, but
  -- prep_day rows are fixed-rate support dates and must not inflate tour
  -- quote scheduled_days or weekly multiplier calculations.
  ...
  LEFT JOIN public.job_date_types jdt
    ON jdt.job_id = _job_id
   AND jdt.date = scheduled_date.date_value::date
   AND jdt.type = 'prep_day'
...
COMMENT ON FUNCTION public.compute_tour_job_rate_quote_2025(uuid,uuid) IS
  'Calculates tour job rate quotes for technicians. Prep-day job_date_types are excluded from scheduled span, scheduled_days, rehearsal/standard day counts, and tour multiplier math because prep days are paid through fixed-rate prep-day timesheets.';
