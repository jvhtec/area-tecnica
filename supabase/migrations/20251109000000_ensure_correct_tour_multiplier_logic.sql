-- Ensure correct tour multiplier logic is applied
-- This migration ensures the function handles all edge cases correctly
...
BEGIN
  ...
  IF team_member AND tour_group IS NOT NULL THEN
    DECLARE
    ...
    BEGIN
      ...
      IF tech_assigned_dates > 0 AND tech_assigned_dates = total_tour_dates THEN
      ...
      ELSE
        ...
        -- Set cnt to tech_assigned_dates but ensure it's at least 1
        cnt := GREATEST(tech_assigned_dates, 1);
        mult := 1.0;
        per_job_multiplier := 1.0;
      END IF;
    END;
  ...
  RETURN jsonb_build_object(
    ...
    'breakdown', jsonb_build_object(
      ...
      'debug_total_tour_dates', (SELECT count(DISTINCT j.id) FROM jobs j WHERE j.job_type = 'tourdate' AND j.tour_id = tour_group AND j.status NOT IN ('cancelled', 'deleted') AND (SELECT iso_year FROM iso_year_week_madrid(j.start_time)) = y AND (SELECT iso_week FROM iso_year_week_madrid(j.start_time)) = w),
      'debug_tech_assigned_dates', (SELECT count(DISTINCT j.id) FROM jobs j JOIN job_assignments a ON a.job_id = j.id WHERE a.technician_id = _tech_id AND j.job_type = 'tourdate' AND j.tour_id = tour_group AND j.status NOT IN ('cancelled', 'deleted') AND (SELECT iso_year FROM iso_year_week_madrid(j.start_time)) = y AND (SELECT iso_week FROM iso_year_week_madrid(j.start_time)) = w)
    )
  );
END;
$function$;
