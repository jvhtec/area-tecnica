-- Split the free-text "waves/outboard" fields into a structured, multi-select
-- waves model list plus a separate free-text outboard notes field. Applies to
-- both FOH and MON sides, on the artist requirements table and on both gear
-- setup tables (global + per-stage).

alter table public.festival_artists
  add column if not exists foh_waves_models text[] not null default '{}'::text[],
  add column if not exists foh_outboard text,
  add column if not exists mon_waves_models text[] not null default '{}'::text[],
  add column if not exists mon_outboard text;
...
-- Preserve existing free-text values as outboard notes (the waves model
-- itself can't be reliably inferred from unstructured text).
update public.festival_artists
set foh_outboard = coalesce(foh_outboard, nullif(trim(foh_waves_outboard), ''))
where foh_waves_outboard is not null;

update public.festival_artists
set mon_outboard = coalesce(mon_outboard, nullif(trim(mon_waves_outboard), ''))
where mon_waves_outboard is not null;

update public.festival_gear_setups
set foh_outboard = coalesce(foh_outboard, nullif(trim(foh_waves_outboard), ''))
where foh_waves_outboard is not null;
...
update public.festival_stage_gear_setups
set foh_outboard = coalesce(foh_outboard, nullif(trim(foh_waves_outboard), ''))
where foh_waves_outboard is not null;
...
alter table public.festival_artists
  drop column if exists foh_waves_outboard,
  drop column if exists mon_waves_outboard;
...
alter table public.festival_artists
  add constraint festival_artists_foh_waves_models_check
    check (foh_waves_models <@ array['server_one','extreme','titan','livebox','fourier','axis_one','axis_scope']::text[]),
  add constraint festival_artists_mon_waves_models_check
    check (mon_waves_models <@ array['server_one','extreme','titan','livebox','fourier','axis_one','axis_scope']::text[]);

alter table public.festival_gear_setups
  add constraint festival_gear_setups_foh_waves_models_check
    check (foh_waves_models <@ array['server_one','extreme','titan','livebox','fourier','axis_one','axis_scope']::text[]),
  add constraint festival_gear_setups_mon_waves_models_check
    check (mon_waves_models <@ array['server_one','extreme','titan','livebox','fourier','axis_one','axis_scope']::text[]);

alter table public.festival_stage_gear_setups
  add constraint festival_stage_gear_setups_foh_waves_models_check
    check (foh_waves_models <@ array['server_one','extreme','titan','livebox','fourier','axis_one','axis_scope']::text[]),
  add constraint festival_stage_gear_setups_mon_waves_models_check
    check (mon_waves_models <@ array['server_one','extreme','titan','livebox','fourier','axis_one','axis_scope']::text[]);

comment on column public.festival_artists.foh_waves_models is
  'Selected FOH Waves server model(s): server_one, extreme, titan, livebox, fourier, axis_one, axis_scope. Multiple selection allowed.';
comment on column public.festival_artists.foh_outboard is
  'Free-text notes for additional FOH outboard gear.';
comment on column public.festival_artists.mon_waves_models is
  'Selected MON Waves server model(s): server_one, extreme, titan, livebox, fourier, axis_one, axis_scope. Multiple selection allowed.';
comment on column public.festival_artists.mon_outboard is
  'Free-text notes for additional MON outboard gear.';

-- Update the public artist form submission RPC to persist the new fields.
    ...
    set
      ...
      foh_waves_models = coalesce(
        (
          ...
          from jsonb_array_elements_text(nullif(p_form_data->'foh_waves_models', 'null'::jsonb))
        ),
      ...
      mon_waves_models = case
        when coalesce((p_form_data->>'monitors_from_foh')::boolean, festival_artists.monitors_from_foh, false)
          ...
          (
            select array_agg(value::text)
            from jsonb_array_elements_text(nullif(p_form_data->'mon_waves_models', 'null'::jsonb))
          ),
