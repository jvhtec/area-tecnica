-- Waves/outboard now has its own festival/band/mixed provider selector,
-- independent of the FOH/MON console provider.

alter table public.festival_artists
  add column if not exists foh_waves_provided_by public.provider_type not null default 'festival'::public.provider_type,
  add column if not exists mon_waves_provided_by public.provider_type not null default 'festival'::public.provider_type;

comment on column public.festival_artists.foh_waves_provided_by is
  'Who provides the FOH Waves server/outboard: festival, band, or mixed.';
comment on column public.festival_artists.mon_waves_provided_by is
  'Who provides the MON Waves server/outboard: festival, band, or mixed.';
