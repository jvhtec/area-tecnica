-- Add proper availability and conflict checking
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      -- Check for unavailability (vacation, travel, sick, day_off)
      EXISTS (
        SELECT 1
      ...
      EXISTS (
        ...
        WHERE ja2.technician_id = p.id
          ...
          AND COALESCE(ja2.status, 'invited') NOT IN ('declined', 'cancelled')
  ...
  scored AS (
    SELECT
      ...
      (wr.has_unavailability OR wr.has_job_overlap) AS hard_conflict
