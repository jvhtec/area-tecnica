-- Supersedes 20260428133000_fix_tour_quote_payable_date_multipliers.sql.
-- Rigging dates are date-scoped: they count toward a technician's quote and
-- weekly multiplier only when that technician is assigned to that rigging date
-- or already has an active timesheet on it.
      ...
      AND (
        ...
        OR EXISTS (
          ...
          WHERE rja.job_id = _job_id
            ...
            AND rja.assignment_date = jdt.date
        )
        OR EXISTS (
          SELECT 1
    ...
    date_multipliers AS (
      ...
      LEFT JOIN LATERAL (
        ...
        WHERE j.job_type = 'tourdate'
          ...
          AND (
            (COALESCE(ja.single_day, FALSE) AND ja.assignment_date = pd.date)
            OR (
              NOT COALESCE(ja.single_day, FALSE)
              AND pd.type <> 'rigging'
            )
            OR EXISTS (
              SELECT 1
...
COMMENT ON FUNCTION public.compute_tour_job_rate_quote_2025(uuid,uuid) IS
  'Calculates tour job rate quotes. Scheduled non-prep job_date_types are the base payable dates, so expanded show days update quotes before all timesheets exist. Rigging dates count only for technicians assigned to that date or with an active timesheet on it. Weekly multipliers are applied per technician per eligible payable date/week; prep days are fixed-rate timesheets and excluded from quote counts.';
