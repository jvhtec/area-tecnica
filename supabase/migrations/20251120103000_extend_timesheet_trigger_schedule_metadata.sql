...
-- Ensure the trigger is registered on job_assignments table
DROP TRIGGER IF EXISTS trg_create_timesheets_for_assignment ON public.job_assignments;
CREATE TRIGGER trg_create_timesheets_for_assignment
  AFTER INSERT ON public.job_assignments
  FOR EACH ROW
  EXECUTE FUNCTION public.create_timesheets_for_assignment();
