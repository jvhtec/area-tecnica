-- Guest-share access for the Tour Ops Hub.
-- This is additive and keeps public access behind a token-validating RPC.
...
create table if not exists public.tour_guest_links (
  ...
  label text not null default 'External tour link',
...
create index if not exists idx_tour_guest_links_tour_id
  on public.tour_guest_links(tour_id);

create index if not exists idx_tour_guest_links_active
  on public.tour_guest_links(tour_id, expires_at, revoked_at);

alter table public.tour_guest_links enable row level security;

drop policy if exists p_tour_guest_links_select_management on public.tour_guest_links;
drop policy if exists p_tour_guest_links_insert_management on public.tour_guest_links;
drop policy if exists p_tour_guest_links_update_management on public.tour_guest_links;
drop policy if exists p_tour_guest_links_delete_management on public.tour_guest_links;

create policy p_tour_guest_links_select_management
  on public.tour_guest_links for select
  using (public.is_admin_or_management());

create policy p_tour_guest_links_insert_management
  on public.tour_guest_links for insert
  with check (public.is_admin_or_management());

create policy p_tour_guest_links_update_management
  on public.tour_guest_links for update
  using (public.is_admin_or_management())
  with check (public.is_admin_or_management());

create policy p_tour_guest_links_delete_management
  on public.tour_guest_links for delete
  using (public.is_admin_or_management());
...
begin
  if not public.is_admin_or_management() then
...
grant select, insert, update, delete on public.tour_guest_links to authenticated;
...
grant execute on function public.get_tour_guest_payload(text) to anon, authenticated;
