-- Public artist forms need the latest gear setup fields so artist links can
-- present the same console, Waves/outboard, and position context as management.
...
begin
  ...
  where gs.job_id = v_artist.job_id
  limit 1;
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        'id', faf.id,
        'file_name', faf.file_name,
        'file_path', faf.file_path,
        'file_type', faf.file_type,
        'file_size', faf.file_size,
        'uploaded_at', faf.uploaded_at,
        'uploaded_by', faf.uploaded_by,
        'uploaded_by_name', coalesce(
          nullif(trim(coalesce(p.first_name, '') || ' ' || coalesce(p.last_name, '')), ''),
          p.email
        )
      )
      order by faf.uploaded_at desc nulls last
    ),
    '[]'::jsonb
  )
  into v_rider_files
  from public.festival_artist_files faf
  left join public.profiles p on p.id = faf.uploaded_by
  where faf.artist_id = v_form.artist_id;
