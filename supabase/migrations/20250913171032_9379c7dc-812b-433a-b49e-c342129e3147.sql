...
create policy if not exists pm_manage_staffing on staffing_requests
...
create policy if not exists tech_read_own_staffing on staffing_requests
for select using (profile_id = auth.uid());

-- PMs can read/insert events
create policy if not exists pm_read_events on staffing_events
for select using (get_current_user_role() = ANY (ARRAY['admin'::text, 'management'::text]));

create policy if not exists pm_insert_events on staffing_events
