...
-- Keeps the legacy transport_requests.status contract intact for older clients while adding
-- an explicit planning lifecycle and request-scoped logistics-event linkage.
...
-- for a department. Preserve compatibility, but refuse to terminalise a request until the new
-- workflow explicitly marks it completed.
...
-- different job, intentionally detach it rather than producing a stale cross-job request link.
...
-- Backfill pre-existing request/event pairs only where the legacy one-request-per-department
-- model makes the relationship unambiguous: one active request and a single-department event.
with active_request as (
  select tr.job_id, tr.department, min(tr.id) as request_id
  from public.transport_requests tr
  where tr.status = 'requested'
  group by tr.job_id, tr.department
  having count(*) = 1
), single_department_event as (
  select led.event_id, min(led.department) as department
  from public.logistics_event_departments led
  group by led.event_id
  having count(*) = 1
)
update public.logistics_events le
set transport_request_id = ar.request_id
from single_department_event sde
join active_request ar on ar.department = sde.department
where le.id = sde.event_id
  and le.job_id = ar.job_id
  and le.transport_request_id is null
...
begin
  ...
  if jsonb_typeof(coalesce(p_items, '[]'::jsonb)) <> 'array' or jsonb_array_length(coalesce(p_items, '[]'::jsonb)) > 20 then
  ...
  if p_request_id is null then
    ...
    insert into public.transport_requests (
      ...
      p_job_id, p_department, v_actor, nullif(trim(p_description), ''), nullif(trim(p_note), ''), 'requested',
      p_needed_at, nullif(trim(p_origin), ''), nullif(trim(p_destination), ''), p_movement_type, p_priority,
      p_source_type, nullif(trim(p_source_ref), ''), p_is_hoja_relevant, 'requested'
    ...
    if not public.transport_request_is_privileged()
       and (v_request.source_type <> p_source_type or v_request.source_ref is distinct from nullif(trim(p_source_ref), '')) then
      raise exception 'Request source cannot be changed' using errcode = '42501';
    end if;
...
begin
  ...
  if not found then raise exception 'Transport request not found' using errcode = 'P0002'; end if;

  if p_stage in ('reviewing', 'planned', 'confirmed', 'completed') and not public.transport_request_is_privileged() then
    raise exception 'Only logistics/management may advance planning' using errcode = '42501';
  end if;
  if p_stage = 'cancelled' and not public.transport_request_is_privileged()
     and v_request.created_by is distinct from v_actor
     and not (public.get_current_user_role() = 'house_tech' and public.current_user_department() = v_request.department) then
    raise exception 'Permission denied' using errcode = '42501';
  end if;

  update public.transport_requests
  set planning_status = p_stage,
      status = case when p_stage = 'completed' then 'fulfilled' when p_stage = 'cancelled' then 'cancelled' else 'requested' end,
      updated_at = now()
  where id = p_request_id;
end;
$$;
...
begin
  ...
  insert into public.logistics_events(
    event_type, transport_type, event_date, event_time, loading_bay, job_id,
    license_plate, transport_provider, notes, is_hoja_relevant, transport_request_id
  ) values (
    'load', v_transport_type::transport_type, p_load_date, p_load_time, nullif(trim(p_loading_bay), ''), v_request.job_id,
    nullif(trim(p_license_plate), ''), nullif(trim(p_provider), '')::transport_provider_enum,
    coalesce(nullif(trim(p_notes), ''), v_request.description), v_request.is_hoja_relevant, p_request_id
  )
  ...
  insert into public.logistics_events(
    ...
    'unload', v_transport_type::transport_type, p_unload_date, p_unload_time, nullif(trim(p_loading_bay), ''), v_request.job_id,
    nullif(trim(p_license_plate), ''), nullif(trim(p_provider), '')::transport_provider_enum,
    coalesce(nullif(trim(p_notes), ''), v_request.description), v_request.is_hoja_relevant, p_request_id
  )
  on conflict (transport_request_id, event_type) where transport_request_id is not null
  ...
  v_stage := case
    when nullif(trim(p_provider), '') is not null or nullif(trim(p_license_plate), '') is not null then 'confirmed'
    else 'planned'
  end;
  update public.transport_requests
  set planning_status = v_stage, status = 'requested', updated_at = now()
  where id = p_request_id;

  return jsonb_build_object('load_event_id', v_load_id, 'unload_event_id', v_unload_id, 'planning_status', v_stage);
end;
$$;
...
begin
  ...
  if p_job_id is not null and p_department is not null and not public.transport_request_is_privileged()
     and not (v_role = 'house_tech' and v_department = p_department) then
    raise exception 'Permission denied' using errcode = '42501';
  end if;

  select coalesce(jsonb_agg(row_payload order by priority_rank, needed_sort nulls last, created_sort desc), '[]'::jsonb)
  into v_result
  from (
    select
...
-- Truck Planner now owns a dedicated generated request instead of rewriting whichever
-- manual request happens to be newest for the department.
...
begin
  ...
  if not (public.tp_is_office_role() or (v_role = 'house_tech' and public.current_user_department() = p_department)) then
    raise exception 'permission denied' using errcode = '42501';
  end if;
  ...
  if v_request_id is null then
    insert into public.transport_requests(
      ...
      p_job_id, p_department, v_actor, p_description, p_note, 'requested',
      'truck_planner', v_source_ref, 'requested', true
    ) returning id into v_request_id;
  else
    update public.transport_requests
...
-- Grants for overridden RPC remain explicit.
