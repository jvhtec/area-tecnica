-- Apply the payroll tour multiplier rules directly:
-- - prep_day rows are fixed-rate timesheet rows and never count as quote days.
-- - weekly multiplier count is per technician.
-- - rigging dates only count for technicians assigned to those rigging jobs.
...
-- - multi-day tourdate jobs still receive the weekly multiplier on each standard payable day.
