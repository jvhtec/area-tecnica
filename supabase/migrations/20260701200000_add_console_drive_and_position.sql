-- FOH "drive" (how the PA is driven) plus a physical position field for
-- both the FOH drive rack and the MON console. Artist-side requirements
-- only; no counterpart on the gear setup tables since this describes how
-- the artist's show should be routed/positioned, not festival inventory.

alter table public.festival_artists
  add column if not exists foh_drive text,
  add column if not exists foh_drive_position text,
  add column if not exists mon_position text;

alter table public.festival_artists
  add constraint festival_artists_foh_drive_check
    check (foh_drive is null or foh_drive in ('l_r', 'l_r_sub_ff', 'other')),
  add constraint festival_artists_foh_drive_position_check
    check (foh_drive_position is null or foh_drive_position in ('foh', 'sl', 'sr')),
  add constraint festival_artists_mon_position_check
    check (mon_position is null or mon_position in ('sl', 'sr'));

comment on column public.festival_artists.foh_drive is
  'FOH system drive configuration: l_r, l_r_sub_ff, or other.';
comment on column public.festival_artists.foh_drive_position is
  'Physical position of the FOH drive rack: foh, sl, or sr.';
comment on column public.festival_artists.mon_position is
  'Physical position of the monitor console: sl or sr.';
...
begin
  ...
  begin
    ...
    set
      ...
      foh_drive = coalesce(nullif(p_form_data->>'foh_drive', ''), festival_artists.foh_drive),
      foh_drive_position = coalesce(nullif(p_form_data->>'foh_drive_position', ''), festival_artists.foh_drive_position),
      ...
      mon_position = case
        when coalesce((p_form_data->>'monitors_from_foh')::boolean, festival_artists.monitors_from_foh, false)
          then null
        else coalesce(nullif(p_form_data->>'mon_position', ''), festival_artists.mon_position)
      end,
