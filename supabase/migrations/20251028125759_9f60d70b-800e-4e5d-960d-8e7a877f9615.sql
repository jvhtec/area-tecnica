-- Fix house tech overtime calculation to be 1/10 of base rate
-- Also fix the breakdown display bug
...
BEGIN
  ...
  SELECT
    ts.id, ts.technician_id, ts.job_id, ts.worked_hours,
    ts.category_override
  INTO t
  FROM timesheets ts
  ...
  -- 6) Get worked hours
  hours_raw := COALESCE(t.worked_hours, 0);

  -- 7) Round up: if minutes >= 30, count as next full hour
  hours_rounded := CEIL(hours_raw);
  IF (hours_raw - FLOOR(hours_raw)) < 0.5 THEN
    hours_rounded := FLOOR(hours_raw);
  END IF;

  -- 8) Compute base and overtime
