-- 1) Fix RLS for existing tables only (views cannot have RLS enabled)
-- Replace overly-permissive SELECT policies with authenticated-only access
...
-- 2) Fix function search_path warnings to improve security
