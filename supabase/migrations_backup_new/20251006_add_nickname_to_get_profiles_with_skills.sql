-- Add nickname to get_profiles_with_skills so UI can display it
...
BEGIN
  ...
  GROUP BY p.id, p.first_name, p.nickname, p.last_name, p.email, p.role, p.phone, p.dni, p.department, p.assignable_as_tech;
END;
$$;
