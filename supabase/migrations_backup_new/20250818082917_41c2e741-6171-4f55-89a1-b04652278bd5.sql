...
-- Users can only update non-sensitive fields (prevent role changes)
...
WITH CHECK (
  ...
  (profiles.role = (SELECT role FROM public.profiles WHERE id = auth.uid()) OR get_current_user_role() = 'admin')
);
