...
serve(async (req) => {
  ...
  // Call SQL function to group timesheets into contiguous date blocks
  const { data: grouped, error: groupErr } = await supabase.rpc("group_timesheet_assignments", {
    p_technician_id: tid,
  ...
  if (groupErr) {
    return new Response("Failed to group timesheets", { status: 500 });
  }

  const blocks = (grouped ?? []) as TimesheetBlock[];
  ...
  const jobIds = Array.from(new Set(blocks.map((b) => b.job_id)));
