-- Fix tour rates to use custom_tech_rates for ALL technicians (not just house_tech role)
-- This allows both house_tech and technician roles to have custom tour rates
...
BEGIN
  ...
  -- Base rate lookup - FIXED: Check custom_tech_rates for ALL technicians (both house_tech and technician roles)
  ...
  IF cat = 'responsable' THEN
    ...
    WHERE profile_id = _tech_id;
  ELSE
    -- For tecnico/especialista, use tour_base_other_eur
    SELECT COALESCE(tour_base_other_eur, base_day_eur) INTO base
    FROM custom_tech_rates
    WHERE profile_id = _tech_id;
  END IF;

  -- If custom rate found, mark it
  IF base IS NOT NULL THEN
    has_custom_rate := TRUE;
  ELSE
    -- No custom rate, use standard rate cards
...
COMMENT ON FUNCTION compute_tour_job_rate_quote_2025 IS
'Calculates tour job rate quotes for technicians. Now properly checks custom_tech_rates for BOTH house_tech and technician roles. Falls back to rate_cards_tour_2025 if no custom rate is set.';
