...
- 2026-05-16: Phase 1 completed across P1-01 through P1-05. Tour scheduling PDF exporters compile without `@ts-nocheck`; the `src` `as any` count dropped from the post-P0 baseline of `529` to `339`; dynamic Supabase/task/Flex/staffing access now uses narrowed table unions or typed wrappers; festival gear JSON normalization is centralized in `src/utils/festivalGearMappers.ts`; and `tsconfig.app.json` now enables `noImplicitAny`. Local validation passed with `npm run typecheck`, `npm run lint`, `npm run test:run`, `npm run build`, `npm run cap:sync`, and `git diff --check`.
