...
- 2026-05-17: P3-05 completed by splitting `src/features/festival-management/useFestivalManagementVm.ts` from `1354` lines into a `222` line composition hook plus feature-local query services, command wrappers, derived selectors, and focused hooks for job data, documents, Flex controls, map previews, print actions, administrative commands, and WhatsApp actions. Review follow-up moved job/document state onto TanStack Query, made festival date derivation and Flex date payloads `Europe/Madrid` aware, and added upload cleanup when job document metadata persistence fails. Focused coverage now includes selector behavior for stage/date/rider/Flex/WhatsApp derivations and Playwright smoke coverage for the festival management overview plus the public blank artist requirements form. Local validation passed with `npm run typecheck`, focused Vitest coverage, focused Playwright coverage, `npm run lint`, `npm run test:critical`, `npm run test:run`, `npm run build`, `npm run cap:sync`, `npm run test:e2e`, and `git diff --check`.
- 2026-05-17: P3-06 completed by splitting the assignment matrix route and render path into focused page controls/reminder helpers, scroll virtualization state, technician ordering/ranking state, matrix dialogs, and cell-level removal/dialog/tooltip modules. The largest matrix files are now under the roadmap limit: `src/pages/JobAssignmentMatrix.tsx` is `697` lines, `src/components/matrix/OptimizedAssignmentMatrix.tsx` is `595` lines, `src/components/matrix/optimized-assignment-matrix/OptimizedAssignmentMatrixView.tsx` is `434` lines, and `src/components/matrix/OptimizedMatrixCell.tsx` is `578` lines. Focused coverage includes the matrix page/unit suites, optimized matrix view/cell suites, optimized matrix data hook coverage, and Playwright smoke coverage for assignment matrix plus staffing recommendations. Local validation passed with `npm run typecheck`, focused Vitest coverage, focused Playwright coverage, `npm run lint`, `npm run test:critical`, `npm run test:run`, `npm run build`, `npm run cap:sync`, `npm run test:e2e`, and `git diff --check`.
- 2026-05-17: P3-07 completed by reducing `supabase/functions/push/broadcast.ts` from `1293` lines to a `227` line router that owns shared context, routing overrides, target loading, payload assembly, and final delivery while moving event-family logic into `supabase/functions/push/broadcast/families/`, shared recipient/delivery helpers into `broadcast/`, and pure Spanish copy builders into `broadcast/messages/`. Representative section coverage now lives in `supabase/functions/push/broadcast/__tests__/eventMessages.test.ts` for job, assignment, logistics, festival, task, and tour-date message families. Local validation passed with `npm run typecheck`, focused event-message Vitest coverage, `npm run lint:functions`, `npm run lint`, `npm run test:critical`, `npm run test:run`, `npm run build`, `npm run cap:sync`, `npm run test:e2e`, and `git diff --check`.
