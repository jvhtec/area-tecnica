...
Legacy tour roots with known UUID columns are adopted. Legacy job and tour-date `flex_folders` rows are also matched to semantic keys by folder type, department, source department, parent, and crew-call identity before execution, so migration does not recreate the existing tree. Because historical tour-root child elements were not consistently tracked, recovery does not recreate technical-documentation children under an existing root. Missing known roots, including Estructura, can still be added safely. A partially populated legacy dry-hire year whose root UUID is unknown requires manual parent reconciliation; new year operations are resumable node by node.
