...
Technical roots are normally derived from the union of persisted department selections across all tour jobs. Legacy tours can have an existing Flex root without tour-date jobs or persisted `job_departments`; when that persisted union is empty, reconciliation recovers the technical selection from the tour's stored `flex_<department>_folder_id` values. New tours still require at least one persisted technical selection before the operation acquires a lease. Production, Personnel, Comercial, and Estructura roots always exist. The root Event Folder is also marked with Custom Field 2 (`Gira`, boolean) = `true`; Custom Field 1 (`Descuento %`, float) remains reserved for the project discount.
