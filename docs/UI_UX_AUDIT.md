...
| **H-6 / M-5 — Loading standardization** | 🟡 Broad rollout | Ad-hoc centered `<div><Loader2 animate-spin/></div>` section loaders migrated to the shared `Loading` primitive (adds `role="status"` + `aria-live`; `hideLabel` preserves spinner-only visuals; existing copy kept as the label): `UsersList`, `ArtistTable`, `ArtistFormSubmissionDialog`, `ArtistFormLinksDialog`, `PendingTasksModal`, `MobileArtistConfigEditor`, `MobileArtistFormSheet`, `ResetPasswordForm`. Inline button spinners are covered by `SubmitButton` (L-2). Remaining section loaders (~30) + true skeletons are a tracked follow-up. |
