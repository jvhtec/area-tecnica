...
| **L-2 — Double-submit guard** | 🟡 Broad rollout | Hand-rolled `<Button disabled={isX}>{spinner}</Button>` async buttons migrated to the shared `SubmitButton` (adds `aria-busy` + consistent spinner, removes duplicated markup): auth forms; festival scheduling dialogs; **+ tours** (`EnhancedTourTravelPlanner`, `TourAccommodationsManager`, `TourContactsManager`, `TourSettingsPanel`, `CreateTourDialog`, `BulkTourFolderActions`), **feedback** (`BugReportForm`, `FeatureRequestForm`), **festival** (`ArtistRequirementsForm`, `FestivalManagementDialogs`). ~20 remaining hand-rolled buttons (icon-only / icon-swap / compound patterns) are a tracked follow-up. |
...
| **H-6 / M-5 — Loading standardization** | 🟡 Broad rollout | Ad-hoc centered `<div><Loader2 animate-spin/></div>` section loaders migrated to the shared `Loading` primitive (adds `role="status"` + `aria-live`; `hideLabel` preserves spinner-only visuals; existing copy kept as the label). **Batch 1:** `UsersList`, `ArtistTable`, `ArtistFormSubmissionDialog`, `ArtistFormLinksDialog`, `PendingTasksModal`, `MobileArtistConfigEditor`, `MobileArtistFormSheet`, `ResetPasswordForm`. **Batch 2:** `Dashboard`, `Profile`, `MorningSummary`, `Festivals` (page loaders), `ProjectNotesDialog`, `AchievementsGrid`, `FlexElementSelectorDialog`. Inline button spinners are covered by `SubmitButton` (L-2). `isDark` technician/super-app loaders excluded (C-1, visual QA). ~20 section loaders + true skeletons remain (tracked). |
...
| **Phase 1 — ESLint guardrails** | 🟡 Started | Core-rule guardrails (no new deps): `no-restricted-syntax` **errors** on native `window.confirm/alert/prompt` (locks in **M-1**) and `no-restricted-imports` **errors** on the removed Radix `@/components/ui/toaster` (locks in **H-2**). Both verified to fire with zero current violations. `jsx-a11y` + color rules still need a plugin dependency. |
