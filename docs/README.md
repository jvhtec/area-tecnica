...
- **[Codebase Maintenance Roadmap](plans/codebase-maintenance-roadmap.md)** - phased technical-debt, refactor, type-safety, performance, and ops cleanup backlog
  ...
  - Phase 1 is complete as of 2026-05-16: PDF `@ts-nocheck` suppressions are removed, `noImplicitAny` is enabled, dynamic Supabase wrappers are typed, festival gear JSON mapping is centralized, and the `src` `as any` count is down to `338`.
