...
- Staffing availability requests (`staffing_requests.requested_by`, `staffing_requests.created_at`, `phase = availability`).
- Staffing offers (`staffing_requests.requested_by`, `staffing_requests.created_at`, `phase = offer`).
...
3. `useStaffingMatrixStatuses` now selects and propagates `requested_by` and `created_at` for the latest availability/offer status shown per cell/date.
   Availability requests and offers are separate `staffing_requests` rows distinguished by the `phase` column.
