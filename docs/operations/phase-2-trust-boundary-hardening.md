...
### 5. Durable public Edge Function rate limiting

Public/tokenized endpoints now share a storage-backed limiter instead of
per-isolate maps or informal token validation alone:

- migration `20260624143000_edge_public_rate_limits.sql` adds
  `edge_rate_limit_counters` and `consume_edge_rate_limit(...)`,
  callable only by `service_role`,
- rate-limit buckets store only SHA-256 hashes generated by the Edge Function;
  raw IPs, tokens, emails, and secrets are not persisted, and
- `supabase/functions/_shared/rateLimit.ts` centralizes client bucket hashing,
  RPC consumption, and `Retry-After`/`X-RateLimit-*` headers.

Covered public-token endpoints:

- `csp-report`
- `get-mapbox-token`
- `submit-public-artist-form` (ingress + token-scoped buckets)
- `upload-public-artist-rider` (ingress + token-scoped buckets)
- `delete-public-artist-rider` (ingress + token/document-scoped buckets)
- `staffing-click` (ingress + per-link buckets)
- `tour-guest-document-url` (ingress + token/document-scoped buckets)
- `tech-calendar-ics` (ingress + token/client-scoped buckets)

Wallboard feed/auth endpoints are intentionally deferred from this slice because
TV polling behavior needs a separate, polling-safe threshold and rollout plan.

### 6. Database authorization regression tests
...
- Phase 0 closures (profile privilege trigger, availability/vacation reads,
  `get_job_total_amounts`, `log_activity_as`) remain in place, and
- the durable Edge rate-limit ledger/RPC remains service-role only and enforces
  hashed identifiers.
...
| Public endpoints have durable abuse controls | Mostly complete for high-risk public/tokenized entrypoints — CSP reports, Mapbox token fetches, artist-form submit/upload/delete, staffing click links, tour guest document signing, and calendar ICS now consume durable hashed buckets, with ingress buckets on tokenized routes to prevent token spraying. Wallboard feed/auth remain deferred for a polling-safe rollout. |
