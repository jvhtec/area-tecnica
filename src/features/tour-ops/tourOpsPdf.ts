...
import type { TourOpsDate, TourOpsModel, TourOpsProjection } from "./types";
...
const footer = (pdf: jsPDF, page: number) => {
  ...
  pdf.setFontSize(8);
  pdf.text(`Generado ${format(new Date(), "d MMM yyyy HH:mm", { locale: es })}`, 12, height - 10);
...
export async function generateTourOpsPdf(
  ...
  pdf.setFont("helvetica", "normal");
  pdf.text(
    projection === "guest" ? "Itinerario externo" : projection === "technician" ? "Itinerario de equipo" : "Tour Ops Book",
    18,
    108,
  );
  pdf.setFontSize(10);
  ...
  autoTable(pdf, {
    ...
    body: model.dates.map((date, index) => [
      ...
      date.health.length ? `${date.health.length} avisos` : "OK",
    ]),
    theme: "grid",
    styles: { fontSize: 8, cellPadding: 2 },
    headStyles: { fillColor: RED, textColor: [255, 255, 255] },
