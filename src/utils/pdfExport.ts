...
const cleanTableName = (name: string): string => {
  // Remove (Default), (Override), and similar suffixes
  return name.replace(/\s*\((Default|Override|default|override)\)\s*$/i, '').trim();
};
  ...
  return new Promise<Blob>((resolve) => {
    ...
    const processTables = () => {
      ...
      tables.forEach((table, index) => {
        ...
        let displayName = cleanTableName(table.name);
        
        // For power reports, append PDU info only if not already present
        if (type === 'power' && (table.customPduType || table.pduType)) {
          if (!/\([^)]+\)$/.test(displayName.trim())) {
            displayName = `${displayName} (${table.customPduType || table.pduType})`;
          }
        }
      ...
      // Add summary page with rigging and power information
      if (summaryRows && summaryRows.length > 0) {
        ...
        if (tables[0]?.toolType === 'consumos') {
          ...
          if (powerSummary) {
            ...
            doc.text(`Potencia Total del Sistema: ${powerSummary.totalSystemWatts.toFixed(2)} W`, 14, yPosition);
            yPosition += 7;
            doc.text(`Corriente Total del Sistema: ${powerSummary.totalSystemAmps.toFixed(2)} A`, 14, yPosition);
            yPosition += 10;
          }
          ...
          tables.forEach((table) => {
            table.rows.forEach((row) => {
              if (row.componentName && followspotComponents.some((name) =>
          ...
          // Regular summary table for pesos tool
          doc.setFontSize(16);
          doc.setTextColor(125, 1, 1);
          doc.text("Resumen", 14, yPosition);
          yPosition += 6;
