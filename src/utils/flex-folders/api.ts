...
/**
 * Retrieve the Flex X_AUTH_TOKEN from Supabase secrets and cache it for reuse.
 *
 * The fetched token is stored in the module cache only if no token invalidation
 * occurred while the request was in flight.
 *
 * @returns The Flex `X_AUTH_TOKEN` string.
 * @throws Error if the secret fetch fails or the response does not contain `X_AUTH_TOKEN`.
 */
