...
export class AccommodationSection {
  ...
  for (const accommodation of validAccommodations) {
      // Auto-fetch a hotel image from Wikimedia (free, no API costs)
      try {
        if (accommodation.hotel_name || accommodation.address) {
          const q = accommodation.hotel_name || accommodation.address || '';
          const photos = await (await import('../services/places-image-service')).PlacesImageService.getPhotosForQuery(q, 1, 500, 300, accommodation.coordinates);
          if (photos.length > 0) {
