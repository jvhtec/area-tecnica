                                ...
                                {job?.job_documents && job.job_documents.filter((d: JobDocument) => d.visible_to_tech).length > 0 ? (
                                    <div className="space-y-2">
                                        {job.job_documents.filter((d: JobDocument) => d.visible_to_tech).map((doc) => (
                                            <div key={doc.id} className={`${isDark ? 'bg-[#151820] border-[#2a2e3b]' : 'bg-slate-50 border-slate-200'} border rounded-lg p-4 flex items-start justify-between gap-3`}>
                                                <div className="flex-1 min-w-0 overflow-hidden">
                                                    <div className={`text-sm font-bold ${theme.textMain} truncate mb-1`} title={doc.file_name}>{doc.file_name}</div>
