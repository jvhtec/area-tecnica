...
export function TimesheetReminderSettings({ className }: { className?: string }) {
  ...
  const getDept = (key: DepartmentKey): DeptSetting =>
    settingsMap.get(key) ?? { department: key, auto_reminders_enabled: false, reminder_frequency_days: 1 };
  ...
  const handleEnableAll = async (enabled: boolean) => {
    try {
      for (const { key } of DEPARTMENTS) {
        await mutation.mutateAsync({ department: key, auto_reminders_enabled: enabled });
      }
      toast({
      ...
      <Card className={className}>
        ...
        <CardContent className="flex items-center justify-between gap-4">
          <p className="text-sm text-muted-foreground">
            No se pudieron cargar los ajustes de recordatorios.
          </p>
